...
define variable c-prog-gerado as character no-undo initial "escc0014".
...
def new shared var c-cod-estabel     AS CHAR  format "x(03)"    initial "422"   no-undo.
...
DEFINE BUTTON bt-troca-fornec 
     LABEL "Troca Fornec." 
     SIZE 15 BY 1. 

DEFINE BUTTON bt-gera-ordem 
     LABEL "Gera Ordem Compra" 
     SIZE 15 BY 1. 
...
"Importa Planilha de Necessidades"
     bt-cancelar AT ROW 14.54 COL 23 HELP
"Fechar"
...
DEFINE FRAME f-pg-dig  
    ...
    bt-retirar AT ROW 10 COL 11

    bt-troca-fornec AT ROW 10 COL 31
   
    bt-gera-ordem AT ROW 10 COL 51
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Gera‡Æo de Ordens de Compras da Lista de Necessidades - escc0014"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
DO:
   ...
   ASSIGN c-arquivo-entrada:SCREEN-VALUE IN FRAME f-pg-sel = dir-jr + "\escc0014" + ".xls". 
...
&Scoped-define SELF-NAME bt-troca-fornec
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-troca-fornec C-Win
ON CHOOSE OF bt-troca-fornec IN FRAME f-pg-dig /* Troca Fornecedor */
DO:

   RUN pi-troca-fornec.

   IF (SUBSTRING(cnpj-velho,1,8) <> SUBSTRING(cnpj-novo,1,8)) OR 
      cod-emitente-velho = cod-emitente-novo  THEN DO:

        run utp/ut-msgs.p (input "show":U, input 32582, "Fornecedores Invalidos - Tem que ser entre matriz X filial").
        return no-apply.                  
   ...
   FOR EACH tt-digita WHERE
        tt-digita.cod-emitente = cod-emitente-velho.

       ASSIGN tt-digita.cod-emitente = cod-emitente-novo
              tt-digita.nome-abrev   = nome-abrev-novo.
...
DO:
    ...
    FOR EACH tt-digita WHERE tt-digita.log-erro = NO EXCLUSIVE-LOCK.
        ...
        IF NOT AVAIL param-compra THEN NEXT.

        ASSIGN i-ordem    = param-compra.prox-ord-aut. 

        FIND FIRST ordem-compra WHERE
            ordem-compra.numero-ordem = (i-ordem * 100)
            NO-LOCK NO-ERROR.
        ...
        ASSIGN tt-ordem-compra.numero-ordem = (i-ordem * 100)
        ...
        ASSIGN tt-ordem-compra.qt-solic       = tt-digita.quantidade
               ...
               tt-ordem-compra.narrativa      = "Amg - " + tt-digita.narrativa
        ...
        assign tt-prazo-compra.numero-ordem = tt-ordem-compra.numero-ordem
               ...
               tt-prazo-compra.qtd-do-forn  = tt-digita.quantidade
               tt-prazo-compra.qtd-sal-forn = tt-digita.quantidade
               tt-prazo-compra.data-entrega = tt-digita.dt-entrega
               tt-prazo-compra.quantid-orig = tt-digita.quantidade.

         FIND FIRST item-fornec WHERE 

                item-fornec.cod-emitente = tt-digita.cod-emitente and
                item-fornec.it-codigo    = tt-prazo-compra.it-codigo NO-LOCK NO-ERROR.

         IF AVAIL  item-fornec THEN
             ASSIGN
                tt-prazo-compra.qtd-sal-forn =  tt-prazo-compra.quant-saldo * ( item-fornec.fator-conver / EXP( 10,  item-fornec.num-casa-dec))
        ...
        IF l-erro = NO THEN DO:
           ...
           ASSIGN tt-digita.numero-ordem = (i-ordem * 100)
           ...
           /* Gera‡Æo da Cota‡Æo Autom tica - Aprova‡Æo Autom tica */
           
           FIND FIRST cotacao-item WHERE 
               cotacao-item.it-codigo    = tt-digita.it-codigo    AND
               cotacao-item.cod-emitente = tt-digita.cod-emitente AND
               cotacao-item.numero-ordem = tt-digita.numero-ordem AND
               cotacao-item.seq-cotac    = 1 NO-ERROR.
           ...
           ASSIGN cotacao-item.it-codigo    = tt-digita.it-codigo
                  cotacao-item.cod-emitente = tt-digita.cod-emitente
                  cotacao-item.numero-ordem = tt-digita.numero-ordem
                  cotacao-item.data-cotacao = today
                  cotacao-item.un           = item.un
                  cotacao-item.preco-unit   = tt-digita.preco-item
                  cotacao-item.pre-unit-for = tt-digita.preco-item
                  cotacao-item.preco-fornec = tt-digita.preco-item
                  cotacao-item.mo-codigo    = 0.

           IF tt-digita.perc-ipi > 0 THEN
               ASSIGN cotacao-item.codigo-ipi   = YES
                      cotacao-item.aliquota-ipi = tt-digita.perc-ipi.
           ELSE
               ASSIGN cotacao-item.codigo-ipi   = NO 
                      cotacao-item.aliquota-ipi = 0.

           ASSIGN cotacao-item.codigo-icm       = 1
                  cotacao-item.aliquota-icm     = tt-digita.perc-icm
                  cotacao-item.taxa-financ      = YES
                  cotacao-item.cod-comprado     = tt-digita.usuar-comprador
                  cotacao-item.cot-aprovada     = YES
                  cotacao-item.aprovador        = tt-digita.usuar-aprov-chefe
                  cotacao-item.cod-transp       = tt-digita.cod-transp
                  cotacao-item.usuario          = c-seg-usuario
                  cotacao-item.data-atualiz     = today
                  cotacao-item.hora-atualiz     = STRING(TIME,"HH:MM:SS")
                  cotacao-item.motivo-apr       = "Aprova‡Æo Autom tica por Tabela de Pre‡o"
                  cotacao-item.narrativa        = tt-digita.narrativa
                  cotacao-item.dias-validade    = 10
                  cotacao-item.seq-cotac        = 1
                  cotacao-item.ind-ret-cotacao  = YES
                  cotacao-item.ind-orig-entrada = 1
                  cotacao-item.ind-via-envio    = 1
                  cotacao-item.log-1            = YES
                  cotacao-item.cod-transp       = 1
                  cotacao-item.atualiza-cp      = YES
                  cotacao-item.cod-cond-pag     = tt-digita.cod-cond-pag.

           find last doc-pend-aprov use-index doc-pend-aprov-u no-error.

                if avail doc-pend-aprov then 
                   assign seq-jr1 = doc-pend-aprov.nr-trans + 1.
                else   
                   assign seq-jr1 = 1.
    


           CREATE doc-pend-aprov.

           ASSIGN doc-pend-aprov.ind-tip-doc       = 3
                  doc-pend-aprov.cod-aprov         = cotacao-item.aprovador
                  doc-pend-aprov.ind-situacao      = 2
                  doc-pend-aprov.sequencia         = 1
                  doc-pend-aprov.it-codigo         = cotacao-item.it-codigo
                  doc-pend-aprov.numero-ordem      = cotacao-item.numero-ordem
                  doc-pend-aprov.dt-aprova         = today
                  doc-pend-aprov.dt-geracao        = today
                  doc-pend-aprov.narrativa-apr     = cotacao-item.narrativa
           /*       doc-pend-aprov.cod-aprov-altern  = cotacao-item.aprovador*/
                  doc-pend-aprov.cod-usuario       = cotacao-item.usuario
                  doc-pend-aprov.cod-emitente      = cotacao-item.cod-emitente
                  doc-pend-aprov.ind-tip-aprov     = 4 
                  doc-pend-aprov.seq-aprov         = 10 
                  doc-pend-aprov.sit-aprov         = yes
                  doc-pend-aprov.cod-tip-aprov     = "10" 
                  doc-pend-aprov.aprov-aut         = yes
                  doc-pend-aprov.num-faixa         = 10 
                  doc-pend-aprov.sc-codigo         = item.sc-codigo
                  substring(doc-pend-aprov.char-2,1,12) = c-seg-usuario
                  substring(doc-pend-aprov.char-2,25,3) = tt-digita.cod-estabel
                  doc-pend-aprov.nr-trans          = seq-jr1
                  doc-pend-aprov.prioridade        = 10.


           /*Grava Dados restantes da Ordem de Compra */
           ...
           IF AVAIL ordem-compra THEN DO:
               ...
               ASSIGN ordem-compra.cod-comprado  = cotacao-item.cod-comprado
                      ordem-compra.situacao      = 3
                      ordem-compra.cod-emitente  = cotacao-item.cod-emitente
                      ordem-compra.preco-unit    = cotacao-item.preco-unit  
                      ordem-compra.pre-unit-for  = cotacao-item.pre-unit-for
                      ordem-compra.preco-fornec  = cotacao-item.preco-fornec
                      ordem-compra.preco-orig    = cotacao-item.preco-fornec
                      ordem-compra.mo-codigo     = cotacao-item.mo-codigo.

               IF tt-digita.perc-ipi > 0 THEN
                   ASSIGN ordem-compra.codigo-ipi       = YES
                          ordem-compra.aliquota-ipi     = tt-digita.perc-ipi.
               ELSE                                     
                   ASSIGN ordem-compra.codigo-ipi       = NO 
                          ordem-compra.aliquota-ipi     = 0.
                                                        
               ASSIGN ordem-compra.codigo-icm           = 1
                      ordem-compra.aliquota-icm         = tt-digita.perc-icm
                      ordem-compra.cod-transp           = tt-digita.cod-transp
                      ordem-compra.taxa-financ          = cotacao-item.taxa-financ
                      ordem-compra.cod-cond-pag         = cotacao-item.cod-cond-pag
                      ordem-compra.impr-ficha           = YES
                      ordem-compra.usuario              = cotacao-item.usuario
                      ordem-compra.data-atualiz         = cotacao-item.data-atualiz
                      ordem-compra.hora-atualiz         = cotacao-item.hora-atualiz
                      ordem-compra.cod-estabel          = tt-digita.cod-estabel
                      ordem-compra.tp-despesa           = item.tp-desp-padrao
                      ordem-compra.qt-acum-nec          = tt-digita.quantidade
                      ordem-compra.cons-mrp             = YES
                      ordem-compra.cons-pmp             = YES
                      ordem-compra.ep-codigo            = i-ep-codigo-usuario
                      ordem-compra.conta-contabil       = item.ct-codigo + item.sc-codigo 
                      ordem-compra.pend-aprov           = 1
                      ordem-compra.sit-ordem-contrat    = 1
                      ordem-compra.prioridade-aprov     = 1
                      ordem-compra.origem-aprov         = 1
                      ordem-compra.dat-ordem            = TODAY
                      ordem-compra.data-cotacao         = TODAY
                      ordem-compra.gera-edi             = YES.
               ...
               IF AVAIL prazo-compra THEN DO:
                   ...
                   ASSIGN prazo-compra.qtd-do-forn      = tt-digita.quantidade
                          prazo-compra.data-orig        = TODAY
                          prazo-compra.situacao         = 3
                          prazo-compra.cons-mrp         = YES
                          prazo-compra.cons-pmp         = YES
                          prazo-compra.nr-estrut        = 1
                          prazo-compra.qtd-do-forn      = tt-digita.quantidade
                          prazo-compra.qtd-sal-forn     = tt-digita.quantidade
                          prazo-compra.data-entrega     = tt-digita.dt-entrega
                          prazo-compra.quantid-orig     = tt-digita.quantidade.
                    ...
                    IF AVAIL  item-fornec THEN
                        ASSIGN
                           prazo-compra.qtd-sal-forn =  tt-prazo-compra.quant-saldo * ( item-fornec.fator-conver / EXP( 10,  item-fornec.num-casa-dec))
                           prazo-compra.qtd-do-forn  =  tt-prazo-compra.qtd-sal-forn.

              
            
               END. 

           END. /*fim Grava Dados restantes da Ordem de Compra */
           
           FIND FIRST necessidade-oc WHERE
               necessidade-oc.cod-estabel = tt-digita.cod-estabel AND
               necessidade-oc.it-codigo   = tt-digita.it-codigo
               EXCLUSIVE-LOCK NO-ERROR.
...
DO:
    ...
    FOR EACH tt-lista NO-LOCK.
        ...
        ASSIGN tt-digita.cod-estabel  = c-cod-estabel:SCREEN-VALUE IN FRAME f-pg-sel
               ...
               tt-digita.narrativa    = tt-lista.narrativa
         ...
         find first item where 
          ...
          FOR EACH tt-fornec EXCLUSIVE-LOCK.
              DELETE tt-fornec.
          END.
          ...
          FOR EACH am-cc-it-tab-preco WHERE
                   am-cc-it-tab-preco.cod-estabel  = tt-digita.cod-estabel  AND
                   am-cc-it-tab-preco.cod-emitente = tt-digita.cod-emitente AND
                   am-cc-it-tab-preco.it-codigo    = tt-digita.it-codigo   AND   
                   am-cc-it-tab-preco.perc-rat-compra > 0
                   USE-INDEX ITEM
                   NO-LOCK ,

              EACH am-cc-tab-preco OF am-cc-it-tab-preco WHERE
                   am-cc-tab-preco.dt-valid-ini <= TODAY AND
                   am-cc-tab-preco.dt-valid-fim >= TODAY AND     
                   am-cc-tab-preco.aprovada      = YES   
                   NO-LOCK.

              ASSIGN soma-perc = soma-perc + am-cc-it-tab-prec.perc-rat-compra
                     soma-jr   = soma-jr + 1.

              FIND FIRST emitente WHERE
                  emitente.cod-emitente = am-cc-it-tab-preco.cod-emitente
                  NO-LOCK NO-ERROR.  

              CREATE tt-fornec.

              ASSIGN tt-fornec.cod-emitente      = am-cc-it-tab-preco.cod-emitente
                     tt-fornec.nome-abrev        = IF AVAIL emitente THEN emitente.nome-abrev ELSE ""
                     tt-fornec.it-codigo         = am-cc-it-tab-preco.it-codigo
                     tt-fornec.perc-rat-compra   = am-cc-it-tab-preco.perc-rat-compra
                     tt-fornec.sequencia         = soma-jr
                     tt-fornec.cod-estabel       = am-cc-it-tab-preco.cod-estabel
                     tt-fornec.nr-tab-preco      = am-cc-it-tab-preco.nr-tab-preco
                     tt-fornec.cod-transp        = am-cc-tab-preco.cod-transp
                     tt-fornec.usuar-aprov-chefe = am-cc-tab-preco.usuar-aprov-chefe.

          END.


          IF soma-jr = 0 THEN DO:
              ASSIGN mens-erro = "Item sem Tabela de Pre‡os Aprovada".
              RUN pi-erro-planilha.
              NEXT.
          END.

          IF soma-perc <> 100 THEN DO:
              ASSIGN mens-erro = "Percentual de Rateio de Compra Diferente de 100%".
              RUN pi-erro-planilha.
              NEXT.
          END.

          ASSIGN qtde-dec  = tt-lista.quantidade
                 seq-jr    = 0
                 soma-qtde = 0.

          FOR EACH tt-fornec EXCLUSIVE-LOCK.

              ASSIGN tt-fornec.quantidade = INT (qtde-dec * (tt-fornec.perc-rat-compra / 100))
                     soma-qtde = soma-qtde + tt-fornec.quantidade.

          END.

          ASSIGN difer-qtde = qtde-dec - soma-qtde.

          IF difer-qtde <> 0 THEN DO:

              FIND FIRST tt-fornec EXCLUSIVE-LOCK
                  NO-ERROR.

              IF AVAIL tt-fornec THEN
                  ASSIGN tt-fornec.quantidade = tt-fornec.quantidade + difer-qtde.

          END.

          FOR EACH tt-fornec NO-LOCK.

              FIND FIRST am-cc-it-tab-preco WHERE
                  am-cc-it-tab-preco.cod-estabel  = tt-fornec.cod-estabel  AND
                  am-cc-it-tab-preco.cod-emitente = tt-fornec.cod-emitente AND
                  am-cc-it-tab-preco.nr-tab-preco = tt-fornec.nr-tab-preco AND
                  am-cc-it-tab-preco.it-codigo    = tt-fornec.it-codigo
                  NO-LOCK NO-ERROR.

              IF NOT AVAIL am-cc-it-tab-preco THEN NEXT.
              ...
              FIND FIRST am-usuario WHERE
                  am-usuario.cod-usuario     = c-seg-usuario AND
                  am-usuario.usuar-comprador = YES
                  USE-INDEX usuario
                  NO-LOCK NO-ERROR.
        ...
        IF AVAIL item-uni-estab AND item-uni-estab.cod-comprado <> "" THEN
            ASSIGN    tt-digita.usuar-comprador  = item-uni-estab.cod-comprado.
        ELSE
            ASSIGN    tt-digita.usuar-comprador  = item.cod-comprado.


         
         
              ASSIGN tt-digita.cod-emitente      = am-cc-it-tab-preco.cod-emitente
                     tt-digita.nome-abrev        = tt-fornec.nome-abrev     
                     tt-digita.quantidade        = tt-fornec.quantidade     
                     tt-digita.preco-item        = am-cc-it-tab-preco.preco-item      
                     tt-digita.cod-cond-pag      = am-cc-it-tab-preco.cod-cond-pag    
                     tt-digita.cod-transp        = tt-fornec.cod-transp    
                     tt-digita.nr-tab-preco      = am-cc-it-tab-preco.nr-tab-preco    
                     tt-digita.perc-icms         = am-cc-it-tab-preco.perc-icms       
                     tt-digita.perc-ipi          = am-cc-it-tab-preco.perc-ipi        
                     tt-digita.perc-rat-compra   = am-cc-it-tab-preco.perc-rat-compra 
                   /*  tt-digita.usuar-comprador   = IF AVAIL am-usuario THEN am-usuario.cod-usuario ELSE "" */
                     tt-digita.log-erro          = NO
                     tt-digita.gerado-ordem      = NO 
                     tt-digita.sequencia         = seq-jr
                     tt-digita.usuar-aprov-chefe = tt-fornec.usuar-aprov-chefe.
                         
              ASSIGN tem-reg = 1.

          END.  /* FOR EACH tt-fornec */
   ...
   IF (erro-jr = 0 AND pesq-jr:SCREEN-VALUE IN FRAME f-pg-sel = "1") or
    ...
    enable bt-retirar bt-troca-fornec bt-gera-ordem with frame f-pg-dig.
...
assign v-cod-prog-gerado = "escc0014".
...
ASSIGN c-arquivo-entrada = dir-jr + "\escc0014" + ".xls". 
         ...
         assign tt-param.usuario              = c-seg-usuario
             ...
             then assign tt-param.arquivo = "".
              ...
              assign tt-param.c-arquivo-jr          = input frame f-pg-sel c-arquivo-entrada   
                     ...
                     tt-param.cod-emitente-jr       = input frame f-pg-sel i-cod-emitente-jr.   

           {include/i-rpexb.i}

           if  session:set-wait-state("general":U) then.

           {include/i-rprun.i ccp/escc0014rp.p} 
...
PROCEDURE pi-troca-fornec.

    ASSIGN cnpj-velho = ""
           cnpj-novo  = ""
           cod-emitente-velho = 0
           cod-emitente-novo  = 0
           nome-abrev-velho   = ""
           nome-abrev-novo    = "".
     ...
     DEFINE BUTTON db-bt-ok AUTO-GO 
          LABEL "&OK" 
          SIZE 10 BY 1
          BGCOLOR 8.
     ...
     DEFINE FRAME db-frame-1
         ...
         cod-emitente-velho LABEL "Cod.Fornecedor" AT ROW 2 COL 16 COLON-ALIGNED view-as FILL-IN size 12 by .88 font 1
         nome-abrev-velho NO-LABEL  AT ROW 2 COL 34  COLON-ALIGNED view-as FILL-IN size 15 by .88 font 1
     
         cod-emitente-novo LABEL "Cod.Novo Fornecedor" AT ROW 4 COL 16 COLON-ALIGNED view-as FILL-IN size 12 by .88 font 1
         nome-abrev-novo NO-LABEL  AT ROW 4 COL 34  COLON-ALIGNED view-as FILL-IN size 15 by .88 font 1
     
     
         db-bt-ok          AT ROW 12.3 COL 2.14  
         db-bt-cancel      AT ROW 12.3 COL 16             
         db-rt-botoes      AT ROW 12.0 COL 1
     
     
     
         SPACE(0.28)
         WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER SIDE-LABELS NO-UNDERLINE 
              THREE-D SCROLLABLE TITLE "Troca Fornecedor" FONT 1
              DEFAULT-BUTTON db-bt-ok CANCEL-BUTTON db-bt-cancel.

     ON "leave":U OF cod-emitente-velho IN FRAME db-frame-1 
     DO:


         FIND FIRST emitente WHERE
             emitente.cod-emitente = int(cod-emitente-velho:SCREEN-VALUE IN FRAME db-frame-1)
             NO-LOCK NO-ERROR.

         IF AVAIL emitente THEN
             ASSIGN cnpj-velho = emitente.cgc
                    nome-abrev-velho:SCREEN-VALUE IN FRAME db-frame-1 = emitente.nome-abrev.

         ELSE

             ASSIGN nome-abrev-velho:SCREEN-VALUE IN FRAME db-frame-1 = "".

         RETURN.
     END.
     

     ON "leave":U OF cod-emitente-novo IN FRAME db-frame-1 
     DO:

         FIND FIRST emitente WHERE
             emitente.cod-emitente = int(cod-emitente-novo:SCREEN-VALUE IN FRAME db-frame-1)
             NO-LOCK NO-ERROR.

         IF AVAIL emitente THEN
             ASSIGN cnpj-novo = emitente.cgc
                    nome-abrev-novo:SCREEN-VALUE IN FRAME db-frame-1 = emitente.nome-abrev.

         ELSE

             ASSIGN nome-abrev-novo:SCREEN-VALUE IN FRAME db-frame-1 = "".

         RETURN.
     END.
     
     
     ON "CHOOSE":U OF db-bt-ok IN FRAME db-frame-1 DO:
     
     
         ASSIGN cod-emitente-velho = int(cod-emitente-velho:SCREEN-VALUE IN FRAME db-frame-1)
                cod-emitente-novo  = int(cod-emitente-novo:SCREEN-VALUE IN FRAME db-frame-1)
                nome-abrev-novo    = string(nome-abrev-novo:SCREEN-VALUE IN FRAME db-frame-1)
                nome-abrev-velho   = string(nome-abrev-velho:SCREEN-VALUE IN FRAME db-frame-1).
     
     
     END.
     
     
     ON "CHOOSE":U OF db-bt-cancel IN FRAME db-frame-1 DO:
