...
/*------------------------------------------------------------------------
File.............: escc0011.w
Description......: Importa Planilha da Tabela de Pre‡os de Fornecedores
Input Parameters : 
Output Parameters: 
...
{include/i-prgvrs.i escc0011 1.00.00.000}
...
DEFINE FRAME f-pg-sel 
    ...
    c-nr-tab-preco label "Nr.Tab. de Pre‡os"
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Importa Planilha da Tabela de Pre‡os de Fornecedores - escc0011"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.29
...
&ANALYZE-RESUME
    ...
    DO: 
       ...
       FIND LAST am-cc-tab-preco WHERE
           am-cc-tab-preco.cod-estabel  = c-cod-estabel:SCREEN-VALUE IN FRAME f-pg-sel AND
           am-cc-tab-preco.cod-emitente = emitente.cod-emitente
           NO-LOCK NO-ERROR.

       IF AVAIL am-cc-tab-preco THEN
           ASSIGN nr-tab-jr = am-cc-tab-preco.nr-tab-preco + 1.
    
       ASSIGN c-nr-tab-preco:SCREEN-VALUE IN FRAME f-pg-sel = STRING (nr-tab-jr). 
...
{utp/ut9000.i "escc0011" "2.00.00.000"}
...
assign v-cod-prog-gerado = "escc0011".
...
/* NÆo Permite Usu rios com Flag = No na tabela am-usuario */  
    ...
    FIND FIRST am-usuario WHERE
        am-usuario.cod-usuario = c-seg-usuario
        NO-LOCK NO-ERROR.

    IF NOT AVAIL am-usuario THEN DO:

       run utp/ut-msgs.p (input "show":U, input 2, "Usu rio Tab.Pre‡o").
       APPLY "CLOSE":U TO THIS-PROCEDURE.
       RETURN "OK".

    END.

    IF AVAIL am-usuario  THEN DO:

      IF am-usuario.usuar-tab-preco = NO THEN DO:
          run utp/ut-msgs.p (input "show":U, input 2015, am-usuario.cod-usuario).
          APPLY "CLOSE":U TO THIS-PROCEDURE.
          RETURN "OK".
      END.
    END.

/* Fim do teste de permissÆo de Usu rios com Flag = No na tabela am-usuario */  
...
do on error undo, return error on stop  undo, return error:
    ...
    /*:T Coloque aqui as valida‡äes das outras p ginas, lembrando que elas devem 
       ...
       IF INT (c-nr-tab-preco:SCREEN-VALUE IN FRAME f-pg-sel) < 1 THEN DO:
          run utp/ut-msgs.p (input "show":U, input 36, "Nr.da Tabela de Pre‡os").
          APPLY "entry" TO c-cod-estabel IN FRAME f-pg-sel.
          return 'ADM-ERROR':U.
       END.
       ...
       IF c-arquivo-jr = ? OR c-arquivo-jr = "" THEN DO:
           run utp/ut-msgs.p (input "show":U, input 326, "Planilha de Conhecimentos").
    ...
    if  tt-param.destino = 2 then
        ...
        assign tt-param.arquivo = session:temp-directory + "escc0011" + ".tmp".
     ...
     assign tt-param.c-arquivo-jr        = input frame f-pg-sel c-arquivo-entrada   
            ...
            tt-param.nr-tab-preco        = input frame f-pg-sel c-nr-tab-preco 
         . 
    ...
    {include/i-rprun.i ccp\escc0011rp.p}
