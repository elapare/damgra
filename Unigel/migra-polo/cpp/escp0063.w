...
/*------------------------------------------------------------------------
File.............: escp0063.w
Description......: Posi‡Æo Di ria - PCP
Input Parameters : 
Output Parameters: 
...
define variable c-prog-gerado as character no-undo initial "escp0063".
...
DEFINE VARIABLE dt-data-inicio        AS DATE       NO-UNDO.

/****************** Defini‡ao de Vari veis de Sele‡Æo do Relat¢rio *********************/ 

def new shared var c-cod-estabel-ini  like nota-fiscal.cod-estabel     format "X(3)"   no-undo.
...
DEFINE FRAME f-pg-sel
   ...
   c-canal-venda-ini label "Canal Venda"
     at row 2 col 17 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1
   
   dt-data-hoje label "Data do Dia"
     at row 4.0 col 17 colon-aligned
     view-as fill-in 
     size 10 by .88
     font 1

    dt-data-ontem LABEL "Data Dia Anterior"
      at row 5.0 col 17 colon-aligned
      view-as fill-in 
      size 10 by .88
      font 1         
   
/*
   IMAGE-3 AT ROW 02.00 COL 32
   IMAGE-4 AT ROW 02.00 COL 37
   IMAGE-5 AT ROW 04.00 COL 32
   IMAGE-6 AT ROW 04.00 COL 37
*/
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Posi‡Æo Di ria - PCP - escp0063"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
ON "LEAVE":U OF dt-data-hoje IN FRAME f-pg-sel DO:

    IF DAY (date(dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel)) <> 1  THEN

        ASSIGN dt-data-ontem:SCREEN-VALUE IN FRAME f-pg-sel = STRING 
              ((date(dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel) - 1)).

    ELSE

        ASSIGN dt-data-ontem:SCREEN-VALUE IN FRAME f-pg-sel = STRING 
            (dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel).
...
assign v-cod-prog-gerado = "escp0063".

IF DAY(dt-data-ontem) <> 1 THEN
   ASSIGN dt-data-ontem = dt-data-ontem - 1.  

def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "Posi‡Æo Di ria - PCP",
...
if return-value = "NOK" then do:
  ...
  return error.
    ...
    IF date(dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel) <
       date(dt-data-ontem:SCREEN-VALUE IN FRAME f-pg-sel) THEN DO:

        MESSAGE "Data de Ontem Invalida"
    ...
    IF day(date(dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel)) = 1 AND
       day(date(dt-data-ontem:SCREEN-VALUE IN FRAME f-pg-sel)) <> 1 THEN DO:
    ...
    IF month(date(dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel)) <>
       MONTH(date(dt-data-ontem:SCREEN-VALUE IN FRAME f-pg-sel)) OR 
       YEAR(date(dt-data-hoje:SCREEN-VALUE IN FRAME f-pg-sel)) <>   
       YEAR(date(dt-data-ontem:SCREEN-VALUE IN FRAME f-pg-sel)) THEN DO:
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "escp0063" + ".tmp".


    assign tt-param.c-cod-estabel-ini  = input frame f-pg-sel c-cod-estabel-ini
           tt-param.dt-data-hoje       = input frame f-pg-sel dt-data-hoje
           tt-param.dt-data-ontem      = input frame f-pg-sel dt-data-ontem
           tt-param.c-canal-venda-ini  = input frame f-pg-sel c-canal-venda-ini
           tt-param.dt-data-inicio     = DATE(MONTH (tt-param.dt-data-hoje),1,YEAR (tt-param.dt-data-hoje))
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cpp/escp0063rp.p".
