...
/*------------------------------------------------------------------------
File.............: escp0030.w
Description......: Bobinas Cortadas no Per¡odo
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 15/08/2007 - 11:11 - jrrcampos
OBS..............: Otimiza‡Æo de Performance em 13/03/2007
------------------------------------------------------------------------*/
define buffer empresa for mgmulti.empresa.
define variable c-prog-gerado as character no-undo initial "escp0030".
...
def new shared var c-hr-trans-ini       AS CHAR FORMAT "99:99:99"    INITIAL "000000"                  NO-UNDO. 
def new shared var c-hr-trans-fim       AS CHAR FORMAT "99:99:99"    INITIAL "240000"                  NO-UNDO. 
...
DEFINE FRAME f-pg-sel
   ...
   c-dt-trans-ini label "Per¡odo"
    ...
    c-hr-trans-fim NO-LABEL
    at row 3 col 59 colon-aligned
    view-as fill-in 
    size 10 by .88
    font 1
    ...
    c-maq-ini label "M quina"
     ...
     c-param-corte  AT ROW 8.2 COL 10 NO-LABEL

     c-param-status AT ROW 8.2 COL 30 NO-LABEL
                            
     c-param-recorte AT ROW 8.2 COL 46

    RECT-21 AT ROW 7.25 COL 9
      "Slitter" VIEW-AS TEXT
     SIZE 08 BY 0.88 AT ROW 7.35 COL 10      

    RECT-22 AT ROW 7.25 COL 29
      "Status" VIEW-AS TEXT
     SIZE 08 BY 0.88 AT ROW 7.35 COL 30      
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Bobinas Cortadas no Per¡odo - escp0030"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
if return-value = "NOK" then do:
  ...
  return error.
    ...
    /* Coloque aqui as valida‡äes das outras p ginas, lembrando que elas
     ...
     IF NOT ((input frame f-pg-sel c-cod-estabel-ini >= "421" AND
        input frame f-pg-sel c-cod-estabel-ini <= "424") OR  
         (input frame f-pg-sel c-cod-estabel-ini >= "411" and
        input frame f-pg-sel c-cod-estabel-ini <= "444"))
         THEN DO:  /*solic-318*/ 

         run utp/ut-msgs.p (input "show", input 56,                  
                            input "Estabelecimento").                
         APPLY "mouse-select-click":U TO im-pg-sel IN FRAME f-relat. 
         APPLY "ENTRY":U TO c-dt-trans-ini IN FRAME f-pg-sel.           
         RETURN "NOK":U.                                             
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "escp0030" + ".tmp".

    assign tt-param.c-cod-estabel-ini = input frame f-pg-sel c-cod-estabel-ini 
           tt-param.c-dt-trans-ini    = input frame f-pg-sel c-dt-trans-ini
           tt-param.i-hr-trans-ini    = INT(substring(input frame f-pg-sel c-hr-trans-ini,1,2)) * 3600 + 
                                        INT(substring(input frame f-pg-sel c-hr-trans-ini,3,2)) * 60   +
                                        INT(substring(input frame f-pg-sel c-hr-trans-ini,5,2)) 
           
           tt-param.c-dt-trans-fim    = input frame f-pg-sel c-dt-trans-fim   
           tt-param.i-hr-trans-fim    = INT(substring(input frame f-pg-sel c-hr-trans-fim,1,2)) * 3600 + 
                                        INT(substring(input frame f-pg-sel c-hr-trans-fim,3,2)) * 60   +
                                        INT(substring(input frame f-pg-sel c-hr-trans-fim,5,2)) 
           ...
           tt-param.c-maq-ini         = input frame f-pg-sel c-maq-ini        
           tt-param.c-maq-fim         = input frame f-pg-sel c-maq-fim        
           tt-param.c-param-corte     = input frame f-pg-sel c-param-corte    
           tt-param.c-param-status    = input frame f-pg-sel c-param-status   
           tt-param.c-param-recorte   = input frame f-pg-sel c-param-recorte   
.
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cpp/escp0030rp.p".
