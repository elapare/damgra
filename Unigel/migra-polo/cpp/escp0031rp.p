/*****************************************************************************
**
**       Programa: escp0031rp.p
**
**       Data....: 03/03/2005
**
**       Autor...: DATASUL S.A.
**
**       Objetivo: Estat°stica de Aceitaá∆o por Mill Roll - Por Bobinas
**
...
define variable c-prog-gerado as character no-undo initial "escp0031RP".
...
DEFINE VARIABLE qdef1-jr   AS INTEGER                   NO-UNDO.
DEFINE VARIABLE qdef2-jr   AS INTEGER                   NO-UNDO.
DEFINE VARIABLE qdef3-jr   AS INTEGER                   NO-UNDO.
...
form ttfil-cod-estabel column-label "Est" format "x(3)" at 001
     ...
     ttfil-qt-prod     COLUMN-LABEL "Qt.Produzida" FORMAT "->>>,>>>,>>9" AT 038
     ttfil-qt-aceita   COLUMN-LABEL "Qt.Aceita"    FORMAT "->>>,>>>,>>9" AT 054
     ttfil-qt-rejeit   COLUMN-LABEL "Qt.Rejeitada" FORMAT "->>>,>>>,>>9" AT 073
...
FORM
    ...
    ttdef-qt-rejeit FORMAT "->>>,>>>,>>9" AT 73
...
assign c-programa     = "escp0031rp"
       c-versao       = "2.00"
       c-revisao      = ".00.000"
       c-titulo-relat = "Estat.de Aceitaá∆o por Mill Roll - Por Bobinas"
       c-sistema      = "".
...
/* Fim da Rotina de Rastreabilidade */    
    ...
    IF qtdbob-jr <> 1 THEN DO:
       ASSIGN ttfil-qt-prod = ttfil-qt-prod + (qtdbob-j9 + qdef1-j9 +
                              qdef2-j9 + qdef3-j9).

       ASSIGN ttfil-qt-aceita = ttfil-qt-aceita + qtdbob-j9.

       ASSIGN ttfil-qt-rejeit = ttfil-qt-rejeit + (qdef1-j9 +
                                qdef2-j9 + qdef3-j9).
    ...
    IF qtdbob-jr = 1 AND cod-def > 0 THEN DO:
    ...
    IF qtdbob-jr <> 1 AND cdef1-jr <> 0 THEN DO:

       ASSIGN c-def-jr = cdef1-jr.
       RUN agrupa-defeitos.
       ASSIGN cdef1-jr = c-def-jr.

       FIND FIRST tt-defeitos WHERE
          ttdef-cod-estabel = movto-estoq.cod-estabel AND
          ttdef-lote        = mill-roll-jr AND
          ttdef-cod-def = cdef1-jr
          USE-INDEX ch-tt-defeitos NO-ERROR.
       ...
       IF NOT AVAIL tt-defeitos THEN DO:
          ...
          ASSIGN ttdef-cod-estabel = movto-estoq.cod-estabel.
          ASSIGN ttdef-lote        = mill-roll-jr.
          ASSIGN ttdef-nr-tabela = ntab1-jr.
          ASSIGN ttdef-cod-def   = cdef1-jr.
       END.

       ASSIGN ttdef-qt-rejeit = ttdef-qt-rejeit + qdef1-j9.

    END.

    IF qtdbob-jr <> 1 AND cdef2-jr <> 0 THEN DO:

       ASSIGN c-def-jr = cdef2-jr.
       RUN agrupa-defeitos.
       ASSIGN cdef2-jr = c-def-jr.  

       FIND FIRST tt-defeitos WHERE
          ttdef-cod-estabel = movto-estoq.cod-estabel AND
          ttdef-lote        = mill-roll-jr AND
          ttdef-cod-def = cdef2-jr
          USE-INDEX ch-tt-defeitos NO-ERROR.
       ...
       IF NOT AVAIL tt-defeitos THEN DO:
          ...
          ASSIGN ttdef-cod-estabel = movto-estoq.cod-estabel.
          ASSIGN ttdef-lote        = mill-roll-jr.
          ASSIGN ttdef-nr-tabela = ntab2-jr.
          ASSIGN ttdef-cod-def = cdef2-jr.
       END.

       ASSIGN ttdef-qt-rejeit = ttdef-qt-rejeit + qdef2-j9.

    END.

    IF qtdbob-jr <> 1 AND cdef3-jr <> 0 THEN DO:

       ASSIGN c-def-jr = cdef3-jr.
       RUN agrupa-defeitos.
       ASSIGN cdef3-jr = c-def-jr.  

       FIND FIRST tt-defeitos WHERE
          ttdef-cod-estabel = movto-estoq.cod-estabel AND
          ttdef-lote        = mill-roll-jr  AND
          ttdef-cod-def = cdef3-jr
          USE-INDEX ch-tt-defeitos NO-ERROR.
       ...
       IF NOT AVAIL tt-defeitos THEN DO:
          ...
          ASSIGN ttdef-cod-estabel = movto-estoq.cod-estabel.
          ASSIGN ttdef-lote        = mill-roll-jr.
          ASSIGN ttdef-nr-tabela = ntab3-jr.
          ASSIGN ttdef-cod-def = cdef3-jr.
       END.

       ASSIGN ttdef-qt-rejeit = ttdef-qt-rejeit + qdef3-j9.
