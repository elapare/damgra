...
/*------------------------------------------------------------------------
File.............: ESCP0007.w
Description......: ESTATISTICA DE PRODUCAO POR TURNO/MµQUINA
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 24/03/04 - 11:08 - jrrcampos
OBS..............: Otimizador de Performance em 07/03/2007
                   17/08/07 este original virou ESCP0051 com  produ‡Æo mensal,
                   este ficou com a versÆo somente produ‡Æo dia

------------------------------------------------------------------------*/
define buffer empresa for mgmulti.empresa.
define variable c-prog-gerado as character no-undo initial "ESCP0007".
...
define temp-table tt-param
    ...
    field c-cod-estabel-fim like movto-estoq.cod-estabel
    field da-dt-trans-ini like movto-estoq.dt-trans
    field da-dt-trans-fim like movto-estoq.dt-trans
    FIELD cod-maquina-ini      AS INT
    FIELD cod-maquina-fim      AS INT
    FIELD turno-ini      AS INT
    FIELD turno-fim      AS INT
    FIELD turma-ini      AS char
    FIELD turma-fim      AS char
.
...
def new shared var c-cod-estabel-ini like movto-estoq.cod-estabel format "x(3)" initial "" no-undo. /*solic-318*/ 
def new shared var c-cod-estabel-fim like movto-estoq.cod-estabel format "x(3)" initial "" no-undo. /*solic-318*/ 
def new shared var da-dt-trans-ini like movto-estoq.dt-trans format "99/99/9999" initial today no-undo.
def new shared var da-dt-trans-fim like movto-estoq.dt-trans format "99/99/9999" initial today no-undo.
def new shared var da-cod-maquina-ini AS INT format ">>9" initial 0 no-undo.
def new shared var da-cod-maquina-fim AS INT format ">>9" initial 999 no-undo.
def new shared var turno-ini AS INT format ">9" initial 0 no-undo.
def new shared var turno-fim AS INT format ">9" initial 9 no-undo.
def new shared var turma-ini AS char format "x(01)" initial " " no-undo.
def new shared var turma-fim AS char format "x(01)" initial "Z" no-undo.
...
"Ajuda"
     ...
     "Data Viewer 2.03"AT ROW 1.8 COL 66.00
...
DEFINE FRAME f-pg-sel
   c-cod-estabel-ini label "Estabel"
   ...
   c-cod-estabel-fim no-label
     at row 1 col 40 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   da-dt-trans-ini label "Dia"
     at row 2 col 19 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   da-dt-trans-fim no-label
     at row 2 col 40 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

    turno-ini label "Turno"
    ...
    turno-fim no-label
    at row 3 col 40 colon-aligned
    view-as fill-in 
    size 11 by .88
    font 1

    da-cod-maquina-ini label "M quina"
     at row 4 col 19 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

    da-cod-maquina-fim no-label
     at row 4 col 40 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1
   ...
   IMAGE-1 AT ROW 04.00 COL 33
   IMAGE-2 AT ROW 04.00 COL 38
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "ESTATISTICA DE PRODUCAO POR TURNO/MµQUINA - ESCP0007"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
assign v-cod-prog-gerado = "ESCP0007".


def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "ESTATISTICA DE PRODUCAO POR MAQUINA",
...
/*run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "ESTATISTICA DE PRODU€ÇO POR TURNO/MµQUINA",
                    input  v-cod-prog-gerado,   
                    output c-tit,
                    output c-arquivo).*/
...
PROCEDURE enable_UI :
   ...
   da-cod-maquina-ini da-cod-maquina-fim
       turno-ini turno-fim turma-ini turma-fim
   ...
   da-dt-trans-ini 
   da-cod-maquina-ini da-cod-maquina-fim
       turno-ini turma-ini turma-fim
   WITH FRAME f-pg-sel IN WINDOW C-Win.
   
   ASSIGN da-dt-trans-fim:VISIBLE IN FRAME f-pg-sel=NO
          c-cod-estabel-fim:VISIBLE IN FRAME f-pg-sel=NO
          turno-fim:VISIBLE IN FRAME f-pg-sel=NO.
...
if return-value = "NOK" then do:
  ...
  return error.
    ...
    /* Coloque aqui as valida‡äes das outras p ginas, lembrando que elas
      ...
      ASSIGN turno-fim = INPUT FRAME f-pg-sel turno-ini
             turno-fim:SCREEN-VALUE = string(INPUT FRAME f-pg-sel turno-ini)
             da-dt-trans-fim = input frame f-pg-sel da-dt-trans-ini
             da-dt-trans-fim:SCREEN-VALUE IN FRAME f-pg-sel = string(input frame f-pg-sel da-dt-trans-ini).  

      IF turno-fim = 2 THEN
          ASSIGN 
          da-dt-trans-fim = input frame f-pg-sel da-dt-trans-ini + 1
          da-dt-trans-fim:SCREEN-VALUE IN FRAME f-pg-sel = string(input frame f-pg-sel da-dt-trans-ini + 1).
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "ESCP0007" + ".tmp".
    ...
    assign tt-param.c-cod-estabel-ini = input frame f-pg-sel c-cod-estabel-ini
           ...
           tt-param.cod-maquina-ini = INPUT FRAME f-pg-sel da-cod-maquina-ini
           tt-param.cod-maquina-fim = INPUT FRAME f-pg-sel da-cod-maquina-fim
           tt-param.turno-ini = INPUT FRAME f-pg-sel turno-ini
           tt-param.turno-fim = INPUT FRAME f-pg-sel turno-fim
           tt-param.turma-ini = INPUT FRAME f-pg-sel turma-ini
           tt-param.turma-fim = INPUT FRAME f-pg-sel turma-fim
.
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cpp/ESCP0007rp.p".
