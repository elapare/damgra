        ...
        IF AVAIL natur-oper THEN DO:
            ...
            ASSIGN ttPedVenda.cod-estabel          = b-ws-ped-venda.cod-estabel WHEN b-ws-ped-venda.cod-estabel <> ''
				   ...
				   */
                   ...
                   ttPedVenda.nome-prog            = "WSCRM0100"
                    ...
                    FOR EACH b-ws-ped-item WHERE
                        ...
                        IF FIRST-OF(b-ws-ped-item.nr-sequencia) THEN DO:
                            ...
                            ASSIGN ttPedItem.nr-sequencia            = nr-seq 
                                   ...
                                   ttPedItem.vl-pretab               = IF b-ws-ped-item.vl-preuni = ? THEN b-ws-ped-item.vl-pretab ELSE b-ws-ped-item.vl-preuni
                                   ttPedItem.vl-preori               = IF b-ws-ped-item.vl-preuni = ? THEN b-ws-ped-item.vl-pretab ELSE b-ws-ped-item.vl-preuni /* Sera o valor com encargos enviado pelo CRM */
                            ...
                            ELSE DO:
                                ...
                                IF AVAIL b-ws-ped-venda THEN DO:
                                    FOR EACH RowErrors:
                                        ASSIGN b-ws-ped-venda.erros = b-ws-ped-venda.erros + string(RowErrors.ErrorSequence)
                                             + "|" + STRING(RowErrors.ErrorNumber) + "|" + 
                                            replace(RowErrors.ErrorDESCRIPTION,";",",") + "(" + b-ws-ped-item.it-codigo + ")|" /*+ replace(RowErrors.ErrorHELP,";",",")*/ + ";".
                                    END.
                                    {ungws/wscrm0100a-msg.i}
                                END.
                            END.
            ...
            IF NOT CAN-FIND(FIRST RowErrors WHERE RowErrors.ErrorSubType = "Error":U) THEN DO:
                FIND FIRST ped-venda
                     WHERE ped-venda.nr-pedido  = ttpedvenda.nr-pedido NO-LOCK NO-ERROR.

                IF (c_descr_erro = "" OR b-ws-ped-venda.orcamento = 1) AND AVAIL ped-venda THEN DO:

                    FIND FIRST b-ped-compl
                         WHERE b-ped-compl.nr-pedido = ped-venda.nr-pedido NO-LOCK NO-ERROR.
                    ...
                    /* registra dados especificos do CABEÄALHO */
                    /* {ungws/wscrm0100f.i} UNIGEL COMERI */
...
-------------------------------------------------------------*/
    ...
    END.
    ELSE DO:
        RUN openQueryStatic IN hDBoPedVenda(INPUT "Defaultpd4000":U).
        RUN emptyRowErrors  IN hDBoPedVenda.
...
END PROCEDURE. /* pi-elimina-pedido */
