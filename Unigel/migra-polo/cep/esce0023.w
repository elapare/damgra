...
/*------------------------------------------------------------------------
File.............: esce0023.w
Description......: An lise de Estoque de Materiais
Input Parameters : 
Output Parameters: 
...
define variable c-prog-gerado as character no-undo initial "esce0023".
...
DEFINE VARIABLE tb-saldo-positivo AS LOGICAL INITIAL YES  
LABEL "Itens com Saldo Positivo"
VIEW-AS TOGGLE-BOX 
SIZE 20 BY .83 
NO-UNDO.

DEFINE VARIABLE tb-saldo-negativo AS LOGICAL INITIAL YES  
LABEL "Itens com Saldo Negativo"
VIEW-AS TOGGLE-BOX 
SIZE 20 BY .83 
NO-UNDO.

DEFINE VARIABLE tb-sem-saldo AS LOGICAL INITIAL YES  
LABEL "Itens Sem Saldo"
VIEW-AS TOGGLE-BOX 
SIZE 20 BY .83 
NO-UNDO.
...
DEFINE FRAME f-pg-sel
   c-cod-estabel label "Estabelecimento"
    ...
    tb-saldo-positivo AT ROW 5.5 COL 8
    tb-saldo-negativo AT ROW 6.3 COL 8
    tb-sem-saldo      AT ROW 7.1 COL 8

    pesq-jr label "Itens Obsoletos"
      at ROW 5.5 col 33 HELP
      "Escolha a Op‡Æo"

    c-mes-ini label "Mˆs Inicial"
      at row 9 col 19 colon-aligned
      view-as fill-in 
      size 06 by .88
      font 1

    c-ano-ini label "Ano Inicial"
      at row 10 col 19 colon-aligned
      view-as fill-in 
      size 06 by .88
      font 1

    c-mes-fim label "Mˆs Final"
      at row 9 col 56 colon-aligned
      view-as fill-in 
      size 06 by .88
      font 1

    c-ano-fim label "Ano Final"
      at row 10 col 56 colon-aligned
      view-as fill-in 
      size 06 by .88
      font 1

    "Per¡odo para" AT ROW 9 COL 34
    "M‚dia de Consumo" AT ROW 10 COL 32

    rect-19 AT ROW 8.3  COL 6 
    RECT-21 AT ROW 8.85 COL 12
    RECT-22 AT ROW 8.85 COL 50
    RECT-23 AT ROW 5    COL 36
    RECT-24 AT ROW 5    COL 6
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "An lise de Estoque de Materiais - esce0023"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "esce0023" + ".tmp".
    ...
    assign tt-param.c-cod-estabel      = input frame f-pg-sel c-cod-estabel   
           ...
           tt-param.tb-saldo-positivo  = input frame f-pg-sel tb-saldo-positivo
           tt-param.tb-saldo-negativo  = input frame f-pg-sel tb-saldo-negativo
           tt-param.tb-sem-saldo       = input frame f-pg-sel tb-sem-saldo     
        .           
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cep/esce0023rp.p".
