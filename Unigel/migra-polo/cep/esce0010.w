...
/*------------------------------------------------------------------------
File.............: esce0010.w
Description......: Resumo Movimentos de Estoque (Fechamento do Mˆs)
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 18/01/2006 - Jos‚ Roberto 
OBS..............: Otimiza‡Æo de Performance em 01/03/2007
------------------------------------------------------------------------*/
def buffer empresa for mgmulti.empresa.
define variable c-prog-gerado as character no-undo initial "esce0010".
...
DEFINE VARIABLE pesq-jr AS INTEGER INITIAL 2
...
DEFINE FRAME f-pg-sel
   ...
   c-ge-codigo-ini  label "Grupo de Estoque"
   ...
   c-cod-depos-ini  label "Dep¢sito"
   ...
   c-mes            label "Mˆs Referˆncia"
     at row 5 col 20 colon-aligned
     view-as fill-in 
     size 5 by .88
     font 1
   
   c-ano            label "Ano Referˆncia"
     at row 6 col 20 colon-aligned
     view-as fill-in 
     size 5 by .88
     font 1
    
    pesq-jr at ROW 8.5 col 20 NO-LABEL 
    
    RECT-21 AT ROW 7.25 COL 18
      "Composi‡Æo de Valores" VIEW-AS TEXT
     SIZE 18 BY 1 AT ROW 7.40 COL 20
   
   IMAGE-1 AT ROW 02.00 COL 41
   IMAGE-2 AT ROW 02.00 COL 47
   IMAGE-3 AT ROW 03.00 COL 41
   IMAGE-4 AT ROW 03.00 COL 47
   IMAGE-5 AT ROW 04.00 COL 41
   IMAGE-6 AT ROW 04.00 COL 47
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Resumo Movimentos de Estoque (Fechamento do Mˆs)- esce0010"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
ON LEAVE OF c-mes IN FRAME f-pg-sel 
DO:

    IF INT (input frame f-pg-sel c-mes) < 1 OR 
       INT (input frame f-pg-sel c-mes) > 12 THEN DO: 
        
        MESSAGE "Mes de Referˆncia Invalido"
...
ON LEAVE OF c-ano IN FRAME f-pg-sel 
DO:

    IF INT (input frame f-pg-sel c-ano) < 2004 THEN DO: 
        
        MESSAGE "Ano de Referˆncia Invalido"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
...
ASSIGN c-mes = INT (MONTH (today)).
ASSIGN c-ano = INT (YEAR  (today)).

assign v-cod-prog-gerado = "esce0010".


def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "Resumo Movimentos de Estoque (Fechamento do Mˆs)",
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "esce0010" + ".tmp".
    ...
    assign tt-param.c-cod-estabel    = input frame f-pg-sel c-cod-estabel   
           ...
           tt-param.c-mes            = input frame f-pg-sel c-mes           
           tt-param.c-ano            = input frame f-pg-sel c-ano           
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cep/esce0010rp.p".
