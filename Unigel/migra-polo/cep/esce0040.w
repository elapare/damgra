...
/*------------------------------------------------------------------------
File.............: esce0040.w
Description......: Movimentos no Dep¢sito EXP
Input Parameters : 
Output Parameters: 
...
define variable c-prog-gerado as character no-undo initial "esce0040".
...
define temp-table tt-param
    ...
    field lote-ini             AS CHAR 
    field lote-fim             AS CHAR 
...
def new shared var i-ge-codigo-fim      AS INTEGER  format ">>9"        initial 49                 no-undo.
...
def new shared var c-lote-ini      AS CHAR     format "x(10)"      initial ""                 no-undo.
def new shared var c-lote-fim      AS CHAR     format "x(10)"      initial "ZZZZZZZZZZZZZZZZ" no-undo.
def new shared var c-usuario-ini      AS CHAR     format "x(10)"      initial ""                 no-undo.
def new shared var c-usuario-fim      AS CHAR     format "x(10)"      initial "ZZZZZZZZZZZZZZZZ" no-undo.
...
DEFINE VARIABLE rs-opcao AS INTEGER INITIAL 2 
     VIEW-AS RADIO-SET VERTICAL 
     RADIO-BUTTONS 
    "Detalhado Estab-Item-Lote-Usu rio", 1,
     "Detalhado Estab-Item-Lote", 2,
     "Resumido Estab-Item", 3,
     "Resumido Estabelecimento", 4
     SIZE 29 BY 3.2 NO-UNDO.
...
DEFINE FRAME f-pg-sel
   ...
   i-ge-codigo-ini label "Grupo de Estoque"
   ...
   c-lote-fim no-label
    ...
    c-usuario-ini label "Usu rio"
      at row 8 col 20 colon-aligned
      view-as fill-in 
      size 17 by .88
      font 1
    c-usuario-fim no-label
   ...
   IMAGE-1 AT ROW 03.00 COL 40
   IMAGE-2 AT ROW 03.00 COL 45
   IMAGE-3 AT ROW 04.00 COL 40
   IMAGE-4 AT ROW 04.00 COL 45
   IMAGE-5 AT ROW 05.00 COL 40
   IMAGE-6 AT ROW 05.00 COL 45
   IMAGE-7 AT ROW 06.00 COL 40
   IMAGE-8 AT ROW 06.00 COL 45
   IMAGE-9 AT ROW 07.00 COL 40
   IMAGE-10 AT ROW 07.00 COL 45
   IMAGE-11 AT ROW 07.00 COL 40
   IMAGE-12 AT ROW 07.00 COL 45
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 10.62.
...
DEFINE FRAME f-pg-par
   ...
   rs-opcao label "Op‡äes de Detalhamento:"
     at ROW 3 col 10 HELP  
     "Escolha a Op‡Æo do Relat¢rio"

   RECT-11 AT ROW 2.8 COL 14
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "EFICIÒNCIA DE CORTE POR SLITTER / PRODUTO - esce0040"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
PROCEDURE enable_UI :
   ENABLE bt-executar bt-cancelar bt-ajuda im-pg-sel  im-pg-par
   ...
   DISPLAY  
       ...
       c-lote-ini c-lote-fim c-usuario-ini c-usuario-fim
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   assign tt-param.usuario              = c-seg-usuario
          ...
          tt-param.ep-codigo            = int(i-ep-codigo-usuario).
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "esce0040" + ".tmp".

    assign tt-param.cod-estabel     = input frame f-pg-sel c-cod-estabel   
           tt-param.fm-codigo-ini   = input frame f-pg-sel c-fm-codigo-ini 
           tt-param.fm-codigo-fim   = input frame f-pg-sel c-fm-codigo-fim 
           ...
           tt-param.usuario-ini     = input frame f-pg-sel c-usuario-ini 
           tt-param.usuario-fim     = input frame f-pg-sel c-usuario-fim 
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cep/esce0040rp.p".
