 ...
 **Objetivo.: Gera Arquivo de Retornos - (TRPAULO)
 **Data.....: 17/01/2006
 ** Otimizado Performance em 14/02/2007
 colocado no-error na leitura da caracteristica do lote - AMGRA - EDSON - 18/07/2008
 ***************************************************************************/
{include/i-prgvrs.i esft0018 2.04.00.000}
...
DEFINE VARIABLE nome-arq-jr1 AS CHARACTER  FORMAT "x(8)" INITIAL "v:/temp/" NO-UNDO.
DEFINE VARIABLE cnpj-jr      AS CHAR       FORMAT 99999999999999   NO-UNDO.
DEFINE VARIABLE data-jrx     AS CHAR       NO-UNDO.
DEFINE VARIABLE ano-jr       AS CHAR       FORMAT 9999   NO-UNDO.
DEFINE VARIABLE mes-jr       AS CHAR       FORMAT 99     NO-UNDO.
DEFINE VARIABLE dia-jr       AS CHAR       FORMAT 99     NO-UNDO.
DEFINE VARIABLE hora-jr      AS CHAR       FORMAT 99     NO-UNDO.
DEFINE VARIABLE minuto-jr    AS CHAR       FORMAT 99     NO-UNDO.
DEFINE VARIABLE nome-arq-jr2 AS CHARACTER  FORMAT "x(4)" INITIAL ".EJC" NO-UNDO.
DEFINE VARIABLE hora-jrx     AS char       NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-br-digita tt-digita.marca tt-digita.pal-nr-pedido tt-digita.pal-nr-sequencia tt-digita.pal-nr-pallet tt-digita.pal-it-codigo tt-digita.pal-cod-refer tt-digita.pal-nro-docto tt-digita.pal-nr-seq-fat tt-digita.pal-largura tt-digita.nr-bobinas tt-digita.pal-peso-liq tt-digita.pal-peso-bru
...
DEFINE VARIABLE i-nr-sequencia-ini AS INT label "Seq" FORMAT ">>>>9"
     INITIAL 0 
     VIEW-AS FILL-IN 
     SIZE 5 BY .66
     TOOLTIP "Informe Seq.do Pedido " NO-UNDO.
...
DEFINE VARIABLE i-nr-sequencia-fim AS INT label "Seq" FORMAT ">>>>9"
     INITIAL 999 
     VIEW-AS FILL-IN 
     SIZE 5 BY .66
     TOOLTIP "Informe Seq.do Pedido " NO-UNDO.
...
DEFINE VARIABLE c-nome-ab-cli AS char FORMAT "x(12)"
     INITIAL ""
     VIEW-AS FILL-IN 
     SIZE 12 BY .66 NO-UNDO.

DEFINE VARIABLE c-arq-saida-jr AS CHAR label "Arq.Ger" FORMAT "x(100)"
     INITIAL "" 
     VIEW-AS FILL-IN 
     SIZE 34.3 BY .66
     TOOLTIP " " NO-UNDO.
...
DEFINE FRAME f-pg-sel
     ...
     i-nr-sequencia-ini  AT ROW 2.6 COL 27.5   COLON-ALIGNED
     c-nr-pedido-fim     AT ROW 2.6 COL 42.72  COLON-ALIGNED  NO-LABEL
     i-nr-sequencia-fim  AT ROW 2.6 COL 55     COLON-ALIGNED  
     ...
     c-nome-ab-cli       AT ROW 6.6 COL 22.72  COLON-ALIGNED  NO-LABEL
     c-arq-saida-jr      AT ROW 6.6 COL 41.51  COLON-ALIGNED
...
/* Pedidos */
   ...
   lstPedidos LABEL "Pedidos/Ordens"
     AT ROW 8.2 col 17 COLON-ALIGNED

   txtInsPedidos LABEL "Inserir Pedido"
    AT ROW 8.2 COL 44 COLON-ALIGNED
    VIEW-AS FILL-IN
    SIZE 12 BY .88
    FONT 1

   txtInsseq LABEL "Seq"
    AT ROW 8.2 COL 62 COLON-ALIGNED
    VIEW-AS FILL-IN
    SIZE 5 BY .88
    FONT 1
...
DO:
    ...
    do transaction on error undo, return no-apply:
        ...
        assign INPUT BROWSE br-digita tt-digita.marca
               ...
               INPUT BROWSE br-digita tt-digita.pal-nr-sequencia
...
ON leave OF c-cod-estabel-ini IN FRAME f-pg-sel 
DO:
    ...
    IF AVAIL estabelec THEN DO:
        ...
        ASSIGN cnpj-jr = estabelec.cgc.

        ASSIGN data-jrx = STRING (TODAY)
               ano-jr   = STRING (YEAR (today))
               mes-jr   = SUBSTRING (data-jrx,4,2)  
               dia-jr   = SUBSTRING (data-jrx,1,2). 

        ASSIGN hora-jrx  = string(time, "HH:MM:SS").
        ASSIGN hora-jr   = SUBSTRING (hora-jrx,1,2).
        ASSIGN minuto-jr = SUBSTRING (hora-jrx,4,2).

        ASSIGN c-arq-saida-jr:SCREEN-VALUE IN FRAME f-pg-sel = 
            nome-arq-jr1 + cnpj-jr + ano-jr + STRING (mes-jr) + STRING (dia-jr) + hora-jr + minuto-jr + nome-arq-jr2.

    END.

    ELSE
        ASSIGN cnpj-jr = ""
               c-arq-saida-jr:SCREEN-VALUE IN FRAME f-pg-sel = "".
...
ASSIGN c-cod-estabel-ini = STRING({cdp\poloestab.i 422}). /*solic-318*/ 
    /*   c-serie       = "20".  */


ASSIGN c-arq-saida-jr = SESSION:TEMP-DIRECTORY
       c-arq-saida-jr = c-arq-saida-jr + STRING("arq-retorno.EJC").
...
FIND FIRST estabelec WHERE
    estabelec.cod-estabel = c-cod-estabel-ini
    NO-LOCK NO-ERROR.
...
ASSIGN c-arq-saida-jr = nome-arq-jr1 + cnpj-jr + ano-jr + STRING (mes-jr) + STRING (dia-jr) + hora-jr + minuto-jr + nome-arq-jr2.
...
{utp/ut9000.i "esft0018" "2.06.00.001"}
...
END.
     ...
     FOR EACH saldo-terc WHERE
         saldo-terc.cod-estabel  = INPUT FRAME f-pg-sel c-cod-estabel-ini and 
         saldo-terc.cod-emitente = INPUT FRAME f-pg-sel i-cod-emitente-ini and 
         saldo-terc.it-codigo  >=  INPUT FRAME f-pg-sel c-it-codigo-ini and 
         saldo-terc.it-codigo  <=  INPUT FRAME f-pg-sel c-it-codigo-fim AND
                 saldo-terc.nro-docto  >=  INPUT FRAME f-pg-sel c-nro-docto-ini and 
                 saldo-terc.nro-docto  <=  INPUT FRAME f-pg-sel c-nro-docto-fim AND
     ...
     FOR EACH tt-lotes NO-LOCK.
         ...
         FOR EACH pallet NO-LOCK WHERE
                ...
                pallet.nr-sequencia >= INT (INPUT FRAME f-pg-sel i-nr-sequencia-ini) and 
                pallet.nr-pedido <= INT (INPUT FRAME f-pg-sel c-nr-pedido-fim) AND
                pallet.nr-sequencia <= INT (INPUT FRAME f-pg-sel i-nr-sequencia-fim)  
                USE-INDEX ITEM ,
                ...
                IF INT (INPUT FRAME f-pg-sel rs-pedido) = 2 
                    ...
                    FOR EACH tt-pedidos-39 NO-LOCK.
                        IF tt-pedidos-39.pedido = pallet.nr-pedido and
                           tt-pedidos-39.nr-sequencia = pallet.nr-sequencia THEN
               ...
               IF varSaldo > 0 THEN DO:
                 ...
                 IF w-larg < INT (INPUT FRAME f-pg-sel c-largura-ini) OR
                   ...
                   ASSIGN tt-Consulta.it-codigo = pallet.it-codigo
                          ...
                          tt-consulta.nr-sequencia = pallet.nr-sequencia
     ...
     for each tt-Consulta NO-LOCK 
         ...
         run pi-acompanhar in h-acomp(input "Gerando Montando Rel: " + string(v-num-reg-lidos)).
              ...
              FIND tt-digita WHERE
                   ...
                   tt-digita.pal-nr-sequencia = tt-consulta.nr-sequencia  AND
              ...
              ASSIGN tt-digita.pal-nr-pedido    = tt-consulta.nr-pedido 
                     tt-digita.pal-nr-sequencia = tt-consulta.nr-sequencia 
...
/*  END. */
  ...
  assign tt-param.usuario              = c-seg-usuario
   ...
   ASSIGN tt-param.c-cod-estabel-ini    = INPUT FRAME f-pg-sel c-cod-estabel-ini
          ...
          tt-param.arq-saida-jr         = INPUT FRAME f-pg-sel c-arq-saida-jr
    ...
    {include/i-rprun.i ftp/esft0018rp.p} 
