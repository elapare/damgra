...
/*------------------------------------------------------------------------
File.............: esft006.w
Description......: N. Fiscais Emitidas no Per¡odo (Mercado - Cliente)
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 24/08/2007 - Jos‚ Roberto / Cl udio
OBS..............: Otimizado Performance em 14/02/2007
------------------------------------------------------------------------*/

define variable c-prog-gerado as character no-undo initial "ESFT006".
...
define temp-table tt-param
    ...
    field da-dt-confirma-ini like it-nota-fisc.dt-confirma
    field da-dt-confirma-fim like it-nota-fisc.dt-confirma
    ...
    FIELD l-amostra          AS LOGICAL
    FIELD l-prime            AS LOGICAL
    FIELD l-off-spc          AS LOGICAL
    FIELD l-outros           AS LOGICAL
    FIELD l-repos            AS LOGICAL
    FIELD mercado            AS INTEGER
    field c-cliente-ini          as char
    field c-cliente-fim          as char
    FIELD transportador-ini      as char
    FIELD transportador-fim      as char
    FIELD grupo-estoque-ini      as INTEGER
     FIELD grupo-estoque-fim     as INTEGER
...
def var v-cod-extens-arq     as char    no-undo. 

/****************** Defini‡ao de Parƒmetros do Relat¢rio *********************/ 

def new shared var txt-pis-cofins as decimal format ">>9.99" label "Pis / Cofins".

/****************** Defini‡ao de Vari veis de Sele‡Æo do Relat¢rio *********************/ 

def new shared var c-cod-estabel-ini like nota-fiscal.cod-estabel format "X(3)" initial "422" no-undo.
def new shared var c-cod-estabel-fim like nota-fiscal.cod-estabel format "X(3)" initial "422" no-undo.
...
def new shared var transportador-fim AS CHAR format "X(5)" initial "ZZZZZ" no-undo.
def new shared var grupo-estoque-ini AS INTEGER initial 0 no-undo.
def new shared var grupo-estoque-fim AS INTEGER initial 99 no-undo.
...
def new shared var da-canal-venda-fim like nota-fiscal.cod-canal-venda format ">>9" initial 99 no-undo.
...
DEFINE VARIABLE l-amostra AS LOGICAL INITIAL yes 
         LABEL "Amostras" 
         VIEW-AS TOGGLE-BOX
         SIZE 15 BY 1.08 NO-UNDO.

DEFINE VARIABLE l-prime AS LOGICAL INITIAL yes 
         LABEL "Primes" 
         VIEW-AS TOGGLE-BOX
         SIZE 15 BY 1.08 NO-UNDO.

DEFINE VARIABLE l-off-spc AS LOGICAL INITIAL yes 
         LABEL "Off Spcs" 
         VIEW-AS TOGGLE-BOX
         SIZE 12 BY 1.08 NO-UNDO.

DEFINE VARIABLE l-outros AS LOGICAL INITIAL yes 
         LABEL "Outros" 
         VIEW-AS TOGGLE-BOX
         SIZE 12 BY 1.08 NO-UNDO. 

DEFINE VARIABLE l-repos AS LOGICAL INITIAL yes 
         LABEL "Reposi‡Æo" 
         VIEW-AS TOGGLE-BOX
         SIZE 12 BY 1.08 NO-UNDO. 
...
DEFINE VARIABLE pesq-jr AS INTEGER INITIAL 1
        ...
        RADIO-BUTTONS 
        "Detalhado (Nota Fiscal)", 1,
        "Detalhado (Transportadora)", 4,
        "Detalhado (Cliente/Transp)", 5,
        "Resumido (Total-Cliente)", 2,
        "Resumido (Cliente-N.Fiscal)", 3,
        "Resumido (Volume)", 6
        
        SIZE 25 BY 3.9 NO-UNDO.


DEFINE VARIABLE mercado AS INTEGER INITIAL 1
        VIEW-AS RADIO-SET HORIZONTAL
        RADIO-BUTTONS 
        "Interno", 1,
        "Externo", 2,
        "Ambos", 3
        SIZE 40 BY 1.0 NO-UNDO.
...
DEFINE FRAME f-relat
    bt-executar AT ROW 17.10 COL 3 HELP
"Dispara a execu‡Æo do relat¢rio"
     bt-cancelar AT ROW 17.10 COL 14 HELP
"Fechar"
     bt-ajuda AT ROW 17.10 COL 70 HELP
"Ajuda"
     ...
     RECT-6 AT ROW 16.23 COL 2.14
     RECT-1 AT ROW 16.81 COL 2
...
DEFINE FRAME f-pg-sel
   c-cod-estabel-ini label "Estabelecimento"
     at row 1 col 21 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   c-cod-estabel-fim NO-LABEL
     at row 1 col 46 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1
   ...
   da-dt-confirma-ini label "Per¡odo"
     at row 2 col 21 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

   da-dt-confirma-fim no-label
     at row 2 col 46 colon-aligned
     view-as fill-in 
     size 11 by .88
     font 1

    da-canal-venda-ini label "Canal Venda"
      at row 3 col 21 colon-aligned
      view-as fill-in 
      size 4 by .88
      font 1

    da-canal-venda-fim no-label
      at row 3 col 46 colon-aligned
      view-as fill-in 
      size 4 by .88
      font 1
    ...
    c-cliente-ini label "Cliente"
    at row 4 col 21 colon-aligned
       view-as fill-in 
       size 11 by .88
       font 1
    
     c-cliente-fim NO-LABEL
       at row 4 col 46 colon-aligned
       view-as fill-in 
       size 11 by .88
       font 1


    transportador-ini label "Transportador"
    at row 5 col 21 colon-aligned
       view-as fill-in 
       size 11 by .88
       font 1
    
     transportador-fim NO-LABEL
   ...
   grupo-estoque-ini label "Grupo de Estoque"
    at row 6 col 21 colon-aligned
       view-as fill-in 
       size 5 by .88
       font 1

     grupo-estoque-fim NO-LABEL
    ...
    l-amostra AT ROW 10.2 COL 12
    l-prime   AT ROW 11.2 COL 12
    l-off-spc AT ROW 10.2 COL 25
    l-outros  AT ROW 11.2 COL 25
    l-repos   AT ROW 12.2 COL 12

    pesq-jr at ROW 9.7 col 44 NO-LABEL 

    mercado at ROW 7.5 col 23 NO-LABEL 


    
    RECT-20 AT ROW 9.2 COL 10
      "Filtros" VIEW-AS TEXT
     SIZE 5 BY 1 AT ROW 8.7 COL 12

    RECT-22 AT ROW 7.2 COL 10
      "Mercado" VIEW-AS TEXT
     SIZE 7 BY 1 AT ROW 6.7 COL 12




    RECT-21 AT ROW 9.2 COL 42
      "Abertura do Relat¢rio" VIEW-AS TEXT
     SIZE 16 BY 1 AT ROW 8.7 COL 44
   
   IMAGE-1 AT ROW 01.00 COL 38
   IMAGE-2 AT ROW 01.00 COL 42
   IMAGE-3 AT ROW 02.00 COL 38
   IMAGE-4 AT ROW 02.00 COL 42
   IMAGE-5 AT ROW 03.00 COL 38
   IMAGE-6 AT ROW 03.00 COL 42
   IMAGE-7 AT ROW 04.00 COL 38
   IMAGE-8 AT ROW 04.00 COL 42
   IMAGE-9 AT ROW 05.00 COL 38
   IMAGE-10 AT ROW 05.00 COL 42
   IMAGE-11 AT ROW 06.00 COL 38
   IMAGE-12 AT ROW 06.00 COL 42
   WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY
   SIDE-LABELS NO-UNDERLINE THREE-D
   AT COL 3 ROW 2.85
   SIZE 76.86 BY 13.12.
...
DEFINE FRAME f-pg-par
   ...
   txt-pis-cofins
     at row 1.35 col 14 colon-aligned
     view-as fill-in
     size 7 by .88
     font 1
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "N. Fiscais Emitidas no Per¡odo - esft006"
   HEIGHT             = 17.5
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
assign v-cod-prog-gerado = "esft006".


def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "N. Fiscais Emitidas no Per¡odo (Mercado - Cliente)",
...
PROCEDURE enable_UI :
   ...
   c-cliente-ini c-cliente-fim
   transportador-ini transportador-fim
   grupo-estoque-ini  grupo-estoque-fim
   da-dt-confirma-ini da-dt-confirma-fim
   da-canal-venda-ini da-canal-venda-fim
   l-amostra l-prime l-off-spc l-outros l-repos
   pesq-jr mercado RECT-20 RECT-21
   WITH FRAME f-pg-sel IN WINDOW C-Win.

   ENABLE IMAGE-1 IMAGE-2 RECT-20 RECT-21
   c-cod-estabel-ini c-cod-estabel-fim
   c-cliente-ini  c-cliente-fim
   transportador-ini  transportador-fim
   grupo-estoque-ini grupo-estoque-fim
   da-dt-confirma-ini da-dt-confirma-fim
   da-canal-venda-ini da-canal-venda-fim
   l-amostra l-prime l-off-spc l-outros l-repos pesq-jr mercado
   WITH FRAME f-pg-sel IN WINDOW C-Win.
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   ELSE if  tt-param.destino = 3 then
       assign tt-param.arquivo = session:temp-directory + "esft006" + ".tmp".
  

    assign tt-param.txt-pis-cofins = input frame f-pg-par txt-pis-cofins
           ...
           tt-param.transportador-ini = input frame f-pg-sel transportador-ini
           tt-param.transportador-fim = input frame f-pg-sel transportador-fim
           tt-param.grupo-estoque-ini = input frame f-pg-sel grupo-estoque-ini
           tt-param.grupo-estoque-fim = input frame f-pg-sel grupo-estoque-fim
           tt-param.da-dt-confirma-ini = input frame f-pg-sel da-dt-confirma-ini
           tt-param.da-dt-confirma-fim = input frame f-pg-sel da-dt-confirma-fim
           ...
           tt-param.mercado            = input frame f-pg-sel mercado
           tt-param.l-amostra          = input frame f-pg-sel l-amostra
           tt-param.l-prime            = input frame f-pg-sel l-prime
           tt-param.l-off-spc          = input frame f-pg-sel l-off-spc
           tt-param.l-outros           = input frame f-pg-sel l-outros
           tt-param.l-repos            = input frame f-pg-sel l-repos
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "ftp/esft0006rp.p".
