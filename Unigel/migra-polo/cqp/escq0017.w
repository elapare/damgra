...
/*------------------------------------------------------------------------
File.............: escq0017.w
Description......: CONTROLE DE MATERIAL COM RESTRI€ÇO
Input Parameters : 
Output Parameters: 
Author...........: DATASUL S.A.
Created..........: 09/09/2005 - 11:11 - Jos‚ Roberto R. Campos
OBS..............: Otimiza‡Æo de Performance em 05/03/2007
------------------------------------------------------------------------*/
def buffer empresa for mgmulti.empresa.
define variable c-prog-gerado as character no-undo initial "escq0017".
...
DEFINE FRAME f-pg-sel
   ...
   c-nr-data-ini label "Data do Per¡odo"
     at row 3 col 17 colon-aligned
     view-as fill-in 
     size 17 by .88
     font 1

   c-nr-data-fim no-label
     at row 3 col 45 colon-aligned
     view-as fill-in 
     size 17 by .88
     font 1

    c-nr-linha-ini label "Linha do Processo"
      at row 4 col 17 colon-aligned
      view-as fill-in 
      size 17 by .88
      font 1

    c-nr-linha-fim no-label
    ...
    c-nr-familia-ini label "Familia de Filme"
      at row 6 col 17 colon-aligned
      view-as fill-in 
      size 17 by .88
      font 1

    c-nr-familia-fim no-label
      at row 6 col 45 colon-aligned
      view-as fill-in 
      size 17 by .88
      font 1

    c-nr-lote-ini label "Nr.Mill Roll"
      at row 7 col 17 colon-aligned
      view-as fill-in 
      size 17 by .88
      font 1

    c-nr-lote-fim no-label
      at row 7 col 45 colon-aligned
      view-as fill-in 
      size 17 by .88
      font 1

   IMAGE-1  AT ROW 03.00 COL 38
   IMAGE-2  AT ROW 03.00 COL 42
   IMAGE-21 AT ROW 04.00 COL 38
   IMAGE-22 AT ROW 04.00 COL 42
   IMAGE-31 AT ROW 05.00 COL 38
   IMAGE-32 AT ROW 05.00 COL 42
   IMAGE-41 AT ROW 06.00 COL 38
   IMAGE-42 AT ROW 06.00 COL 42
   IMAGE-51 AT ROW 07.00 COL 38
   IMAGE-52 AT ROW 07.00 COL 42
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "CONTROLE DE MATERIAL COM RESTRI€ÇO - escq0017"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
assign v-cod-prog-gerado = "escq0017".


def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "CONTROLE DE MATERIAL COM RESTRI€ÇO",
...
PROCEDURE enable_UI :
   ...
   DISPLAY  
   c-cod-estabel-ini 
   c-nr-data-ini c-nr-data-fim
   c-nr-linha-ini c-nr-linha-fim
   c-nr-familia-ini c-nr-familia-fim
   c-it-codigo-ini c-it-codigo-fim
   c-nr-lote-ini c-nr-lote-fim
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "escq0017" + ".tmp".

    assign tt-param.c-cod-estabel-ini = input frame f-pg-sel c-cod-estabel-ini
           tt-param.c-nr-data-ini     = input frame f-pg-sel c-nr-data-ini
           tt-param.c-nr-data-fim     = input frame f-pg-sel c-nr-data-fim
           tt-param.c-nr-linha-ini    = input frame f-pg-sel c-nr-linha-ini
           tt-param.c-nr-linha-fim    = input frame f-pg-sel c-nr-linha-fim
           ...
           tt-param.c-nr-familia-ini  = input frame f-pg-sel c-nr-familia-ini
           tt-param.c-nr-familia-fim  = input frame f-pg-sel c-nr-familia-fim
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cqp/escq0017rp.p".
