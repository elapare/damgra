...
define variable c-prog-gerado as character no-undo initial "escq0051RP".
...
DEFINE VARIABLE OS-FILE AS CHARACTER   NO-UNDO.
    ...
    IF FILE-INFO:PATHNAME = ? THEN DO:
       OS-FILE = "\\ungusb-vap01\Sistemas\DTS\Log_Prd\escq0051.LOG".
    
    END.
    ELSE DO:
        OS-FILE = "m:\dts\log_prd\escq0051.LOG".
    END.
    ...
    FOR EACH tt-am-cq-result-laudo NO-LOCK.
        ...
        IF tt-am-cq-result-laudo.media-result = 0 THEN DO: 
          ...
          OUTPUT TO VALUE(OS-FILE) APPEND.
               PUT UNFORMATTED  
                   "Nota:" ";" tt-am-cq-result-laudo.nr-nota-fis ";"
                   "Seria:" ";"  ";"
                   "Estab:" ";" tt-am-cq-result-laudo.cod-estabel ";"
                   "Exame:" ";" tt-am-cq-result-laudo.descricao ";"                   
                   "Emiss∆o:" ";" string(TODAY) ";" 
                   "Hora:" ";" string(TIME,"hh:mm:ss") ";"  
                   "Usu†rio:" ";" c-seg-usuario ";" 
                   "Laudo:" ";" "Laudo com Resultado Zerado" ";"
...
FOR EACH tt-am-cq-laudo WHERE
    ...
    if c-tem-analise = "N"  and  tt-param.tipo-geracao = 2 AND l-libera-zerado = NO /* c-seg-usuario <> "fgcardo" and c-seg-usuario <> "aschnei" and  c-seg-usuario <> "bapsilv"*/  then do:
       ...
       OUTPUT TO VALUE(OS-FILE) APPEND.
               PUT UNFORMATTED 
                   "Nota:" ";" tt-am-cq-laudo.nr-nota-fis ";"
                   "Serie:" ";" int(tt-am-cq-laudo.serie) ";"
                   "Estab:" ";" tt-am-cq-laudo.cod-estabel ";"
                   "Cliente:" ";" tt-am-cq-laudo.nome-abrev ";"                   
                   "Emiss∆o:" ";" string(TODAY) ";" 
                   "Hora:" ";" string(TIME,"hh:mm:ss") ";"  
                   "Usu†rio:" ";" c-seg-usuario ";" 
                   "Laudo:" ";" "Laudo Sem Resultados" ";"  
    ...
    IF tt-param.tipo-geracao = 2 THEN DO:
        ...
        OUTPUT TO VALUE(OS-FILE) APPEND.
               PUT UNFORMATTED 
                   ...
                   "Hora:" ";" string(TIME,"hh:mm:ss") ";"  
                   "Usu†rio:" ";" c-seg-usuario ";" 
                   "Tem An†lise:" ";" c-tem-analise ";"  
                   "Laudo Zerado:" ";" c-laudo-zerado ";" 
                    SKIP.
       OUTPUT CLOSE.
    ...
    ELSE DO:
        ...
        IF NOT AVAIL tt-bobinas THEN DO:
           ...
           ASSIGN ttbob-lote = lote-rast
                  ttbob-linha = 1
                  ttbob-it-codigo = tt-rastrear.ttras-it-codigo-cons WHEN AVAIL tt-rastrear
                  ttbob-nr-ord-produ = 0
                  ttbob-dt-trans  = data-rast.
...
PROCEDURE pi-gera-e-mail: 
    ...
    IF tt-am-cq-laudo.tipo-envio-email = 1 THEN DO: 
        ...
        IF NOT AVAIL usuar_mestre OR (AVAIL usuar_mestre AND trim(usuar_mestre.cod_e_mail_local) = "") THEN DO: 
        ...
        OUTPUT TO VALUE(OS-FILE) APPEND.
               PUT UNFORMATTED 
                   ...
                   "Hora:" ";" string(TIME,"hh:mm:ss") ";"  
                   "Usu†rio:" ";" c-seg-usuario ";" 
                   "Respons:" ";" (IF trim(c-email-responsavel) = "" THEN "verifique e-mail operador" ELSE c-email-responsavel) ";"  
                   "E-mail:" ";" IF AVAIL polo-laudo-cliente THEN polo-laudo-cliente.end-email ELSE "N∆o tem parametro de laudo cliente" ";" 
                   "Obs:" ";" TRIM(tt-am-cq-laudo.observacao) ";" 
                    SKIP.
       OUTPUT CLOSE.
       ...
       IF NOT AVAIL polo-laudo-cliente THEN RETURN "NOK".
        ...
        ASSIGN 
            ...
            tt-envio.copia       =  (IF TODAY < 08/05/2015 THEN "edson@damgra.com.br,ricardo.polimeno@unigel.com.br" ELSE "")
