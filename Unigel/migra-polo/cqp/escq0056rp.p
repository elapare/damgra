 ...
 **Objetivo.: Laudo de Qualidade - NF de Transferància
 **Data.....: 22/01/2010
 **
 ***************************************************************************/
 {bf/buffersUni2.i}

define variable c-prog-gerado as character no-undo initial "escq0056RP".
...
/*kraft*/
DEFINE TEMP-TABLE tt-plt-prod no-undo
    FIELD dt-prod-plt          AS DATE 
    INDEX chave IS PRIMARY UNIQUE dt-prod-plt.  
...
FOR EACH tt-notas NO-LOCK.
    ...
    ASSIGN esp-min  = 9999
        ...
        FOR EACH tt-digita WHERE
            ...
            tt-digita.nr-pedido   = tt-notas.nr-pedido
            NO-LOCK.
...
IF tt-param.tipo-geracao = 2 and 
    (c-seg-usuario <> "afranco" and
     c-seg-usuario <> "aschnei" and
     c-seg-usuario <> "bapsilv") THEN DO:
...
FOR EACH tt-am-cq-laudo WHERE
    ...
    IF tt-param.tipo-geracao = 1 THEN DO:
        ...
        IF NOT AVAIL pol-param-relatorio THEN DO:
            ...
            ASSIGN pol-param-relatorio.cod-prog-dtsul = "escq0051"                           
                   pol-param-relatorio.cod-usuario    = tt-am-cq-laudo.nr-nota-fis 
                   pol-param-relatorio.mes-refer      = 20                                   
                   pol-param-relatorio.ano-refer      = int(tt-am-cq-laudo.cod-estabel). 
        END.
...
PROCEDURE pi-gera-laudo:
    ...
    */
    ...
    FIND FIRST ped-venda 
             where ped-venda.nr-pedido  = c-nr-pedido-1
             NO-LOCK NO-ERROR.             
                    ...
                    IF AVAIL pallet THEN DO:
                        ...
                        /* Fim da Dureza */  
                                ...
                                ASSIGN lote-rastrear = it-pallet.lote-bobina 
                                 ...
                                 RUN esp/escp033.p (INPUT lote-rastrear,
    ...
    DO: 
        ...
        /* cabeáalho do excel */
         ...
         FIND FIRST usuar_mestre WHERE
             usuar_mestre.cod_usuario = /*tt-param.usuario*/ "alonghi"
             NO-LOCK NO-ERROR.
         ...
         END.
             ...
             ASSIGN tt-am-cq-laudo.pedido = pedido-x1 + " " + pedido-x2.
           ...
           IF (INDEX (ttbob2-it-codigo,"MR",1) <> 0) THEN DO:
            ...
            FIND LAST movto-mat 
             where 
                   ...
                   movto-mat.cod-depos <> "arc" USE-INDEX lote
                   NO-LOCK  NO-ERROR.
...
/* Vou tirar a rotina de leitura dos dados do escq053 para verificar
   se os resultados nos laudos fiquem mais corretos */
   
   
   /*
   
       FOR EACH am-cq-result-analise WHERE
           am-cq-result-analise.nr-ord-produ = ttbob2-nr-ord-produ
           NO-LOCK.
           FIND FIRST tt-analises WHERE
           ...
           ASSIGN ttana-it-codigo = ttbob2-it-codigo
                  RESULT-jr = DEC (am-cq-result-analise.media-result)
                  ttana-result = ttana-result + RESULT-jr
                  ttana-qtd-result = ttana-qtd-result + 1
                  ttana-amostra[ttana-qtd-result] = RESULT-jr no-error
                  minimo-jr = ttana-minimo
                  maximo-jr = ttana-maximo.
       ...
       fim dos dados do escq053 */ 
       
    END.


    /* Procurar an†lises digitadas em lotes (pallets) (Souza Cruz)*/
...
PROCEDURE pi-gera-e-mail: 
    ...
    IF tt-am-cq-laudo.tipo-envio-email = 1 THEN DO: 
        ...
        FIND usuar_mestre WHERE            
             usuar_mestre.cod_usuario = "alonghi" /*tt-am-cq-laudo.cod-usuario*/ 
             NO-LOCK NO-ERROR.
        ...
        OUTPUT TO VALUE(c-arq + "escq0056MAIL.TMP").
...
PROCEDURE pi-gera-e-mail-2: 
        ...
        ASSIGN 
            tt-envio.versao-integracao = 1
            tt-envio.exchange    =  param-global.log-1
            tt-envio.remetente   =  "alexsander.longhi@polofilms.com.br"
            tt-envio.destino     =  "alexsander.longhi@polofilms.com.br"
            tt-envio.copia       =  "aline.schneider@polofilms.com.br," + c-email-responsavel
