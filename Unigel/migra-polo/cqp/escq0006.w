...
/*------------------------------------------------------------------------
File.............: escq0006.w
Description......: RASTREABILIDADE DE PEDIDOS C/DEFEITOS DE QUALIDADE
Input Parameters : 
Output Parameters: 
Author...........: Amgra Jos‚ Roberto.
Created..........: 30/07/2009 - 11:11 - jrrcampos

------------------------------------------------------------------------*/
def buffer empresa for mgmulti.empresa.
define variable c-prog-gerado as character no-undo initial "escq0006".
...
DEFINE FRAME f-pg-sel
   c-cod-estabel-ini label "Estabelecimento"
     at row 2 col 17.2 colon-aligned
     view-as fill-in 
     size 4 by .88
     font 1

   lstPedidos LABEL "Lista de Pedidos"
     AT ROW 4 col 23 COLON-ALIGNED
   ...
   txtApagar LABEL "Apagar"
    ...
    txtseqapagar LABEL "Seq"
     AT ROW 5.5 COL 67 COLON-ALIGNED
     VIEW-AS FILL-IN
     SIZE 5 BY 1
     FONT 1

    bt-ApagarTudo LABEL "Apagar Tudo"
     AT ROW 7 COL 50 COLON-ALIGNED

   rect-19 AT ROW 3.5 COL 10
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "RASTREABILIDADE DE PEDIDOS C/DEFEITOS DE QUALIDADE - escq0006"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
DO:
    ...
    FOR EACH tt-pedidos-06 NO-LOCK WHERE
        tt-pedidos-06.Pedido = INTEGER(txtApagar:SCREEN-VALUE) AND
        tt-pedidos-06.nr-sequencia = INTEGER(txtseqapagar:SCREEN-VALUE):
        DELETE tt-pedidos-06.
    END.
    ...
    FOR EACH tt-pedidos-06 NO-LOCK:
        ASSIGN lstPedidos:SCREEN-VALUE = string(lstPedidos:SCREEN-VALUE) + string(tt-pedidos-06.pedido) + "-" +
                                         string(tt-pedidos-06.nr-sequencia) + CHR(13).
    END.
...
DO:
    ...
    FIND FIRST tt-pedidos-06 NO-LOCK WHERE 
        tt-pedidos-06.Pedido = integer(txtInsPedidos:SCREEN-VALUE) and
        tt-pedidos-06.nr-sequencia = integer(txtInsseq:SCREEN-VALUE)
        NO-ERROR.

    IF NOT AVAIL tt-pedidos-06 THEN DO:
      CREATE tt-pedidos-06.
      ASSIGN tt-pedidos-06.Pedido = integer(txtInsPedidos:SCREEN-VALUE)
             tt-pedidos-06.nr-sequencia = integer(txtInsseq:SCREEN-VALUE).
...
assign v-cod-prog-gerado = "escq0006".


def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "RASTREABILIDADE DE PEDIDOS C/DEFEITOS DE QUALIDADE",
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    assign rs-destino:screen-value  in frame f-pg-imp = "4":U.
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "escq0006" + ".tmp".
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "cqp/escq0006rp.p".
