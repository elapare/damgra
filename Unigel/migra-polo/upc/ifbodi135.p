...
DEFINE VARIABLE OS-FILE AS CHARACTER   NO-UNDO.
DEFINE VARIABLE nf-log    AS CHARACTER   NO-UNDO.
DEFINE VARIABLE serie-log AS CHARACTER   NO-UNDO.
DEFINE VARIABLE estab-log AS CHARACTER   NO-UNDO.
DEFINE TEMP-TABLE tt-aloc-man-ver NO-UNDO LIKE tt-aloc-man.
...
/* Processamento */
FIND FIRST tt-epc
     WHERE tt-epc.cod-event = "ExecutaOutrasIntegracoes" /* "afterEfetivaNota" */
...
IF AVAIL tt-epc THEN DO:
    ...
    IF AVAIL nota-fiscal AND nota-fiscal.ind-tip-nota = 1 THEN DO: /* s¢ nota sistema */
    ...
    IF AVAIL if-natur-oper  THEN DO:
     ...
     /*   RUN inicializaAcompanhamento IN  h-bodi317sd.
       */
        ...
        END.  /*mudei 19/10/2016 s¢ fazer isso para a primeira nota*/
...
PROCEDURE pi-remessa-conta-ordem :
    /*------------------------------------------------------------------------------
  Purpose:     Fatura por conta e ordem da  Polo para o Cliente
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE i-seq-wt-it-docto AS INTEGER     NO-UNDO.
        ...
        FIND FIRST if-natur-oper NO-LOCK
             WHERE if-natur-oper.cod-estab-orig  = ped-venda.cod-estabel
               ...
               AND if-natur-oper.cod-estab-dest  = b2-ped-venda.cod-estabel
               AND if-natur-oper.nat-oper-pedido = if-ped-venda.nat-oper-orig NO-ERROR.
        IF AVAIL if-natur-oper AND if-natur-oper.nat-oper-remes-ct-ordem <> "" THEN DO:
            ...
            RUN criaWtDocto IN h-bodi317sd (INPUT c-seg-usuario,                     /* usu†rio                                */
                                            ...
                                            INPUT TODAY,                             /* data emissao nota                      */
                                            INPUT 0,                                 /* embarque                               */
                                            INPUT if-natur-oper.nat-oper-remes-ct-ordem,/* natureza operacao                      */
                                            INPUT 0,                                 /* canal venda                            */
                                           OUTPUT i-seq-wt-docto,                    /* seq docto                              */
                                           OUTPUT l-proc-ok-aux).                    /* ok Sim/Nao?                            */
            ...
            FIND FIRST b2-natur-oper
                 WHERE b2-natur-oper.nat-operacao = if-natur-oper.nat-oper-remes-ct-ordem NO-LOCK NO-ERROR.
            ...
            FOR EACH b-it-nota-fisc OF nota-fiscal NO-LOCK:
                ...
                RUN criaWtItDocto IN h-bodi317sd(INPUT ?,                                 /*               */
                                                 ...
                                                 INPUT b-it-nota-fisc.it-codigo,          /* item          */
                                                 INPUT "",                                /* referencia    */
                                                 INPUT if-natur-oper.nat-oper-remes-ct-ordem,/* nat. operacao */
                                                OUTPUT i-seq-wt-it-docto,                 /* sequencia     */
                                                OUTPUT l-proc-ok-aux).                    /* erro          */
...
PROCEDURE pi-gera-embarque-polo :
/*------------------------------------------------------------------------------
  Purpose:     Gera Embarque
  Parameters:  <none>
  Notes:       
        ...
        FOR EACH tt-alocar NO-LOCK
           ...
           BREAK BY(tt-alocar.nome-abrev):
            ...
            IF FIRST-OF(tt-alocar.nome-abrev) THEN DO:
                ...
                FOR EACH b3-it-nota-fisc OF nota-fiscal  NO-LOCK,
                   ...
                   WHERE b5-ped-venda.nr-pedido  = if-ped-venda.nr-pedido-relac
                    ...
                    CREATE tt-aloc-man.
                    ASSIGN tt-aloc-man.cdd-embarq  = i-embarque
                           tt-aloc-man.nr-resumo    = nota-fiscal.nr-resumo
                           tt-aloc-man.nome-abrev   = tt-alocar.nome-abrev
                           tt-aloc-man.nr-pedcli    = b3-it-nota-fisc.nr-pedcli 
                           tt-aloc-man.nr-sequencia = b3-it-nota-fisc.nr-seq-ped
                           tt-aloc-man.it-codigo    = b3-it-nota-fisc.it-codigo 
                           tt-aloc-man.cod-estabel  = tt-alocar.cod-estabel
                           tt-aloc-man.cod-depos    = fat-ser-lote.cod-depos
                           tt-aloc-man.cod-localiz  = fat-ser-lote.cod-localiz
                           tt-aloc-man.lote         = fat-ser-lote.nr-serlote
                           tt-aloc-man.cod-refer    = b3-it-nota-fisc.cod-refer
                           tt-aloc-man.qt-a-alocar  = fat-ser-lote.qt-baixada[1]
                           tt-aloc-man.i-sequen     = i-sequencia
                           tt-aloc-man.nr-entrega   = b3-it-nota-fisc.nr-entrega.


                    ASSIGN i-tipo-atend = b3-it-nota-fisc.tipo-atend.


                    /* verifica a quantida a alocar do pedido para ver se tem sujeira */
                    FIND FIRST b3-ped-item 
                         WHERE b3-ped-item.nome-abrev   = b5-ped-venda.nome-abrev
                           AND b3-ped-item.nr-pedcli    = b5-ped-venda.nr-pedcli
                           AND b3-ped-item.nr-sequencia = b3-it-nota-fisc.nr-seq-ped
                           AND b3-ped-item.it-codigo    = b3-it-nota-fisc.it-codigo 
                           AND b3-ped-item.cod-refer    = b3-it-nota-fisc.cod-refer NO-LOCK NO-ERROR.
                    IF AVAIL b3-ped-item AND (b3-ped-item.qt-pedida - b3-ped-item.qt-atendida) < tt-aloc-man.qt-a-alocar THEN DO:
                        RUN pi-sppd309-ped (INPUT b3-ped-item.nome-abrev, INPUT b3-ped-item.nr-pedcli).
                    END.
            ...
            IF LAST-OF(tt-alocar.nome-abrev) THEN DO:
                ...
                RUN pi-recebe-tipo-atend   IN h-eqapi300 (INPUT i-tipo-atend).
...
END PROCEDURE. /* pi-gera-embarque-polo */
    ...
    DO TRANSACTION:
        FOR EACH tt-alocar NO-LOCK
           ...
           BREAK BY(tt-alocar.nome-abrev):
            ...
            FIND FIRST 
                 tt-ped-venda-aux WHERE 
                   tt-ped-venda-aux.i-sequen    = i-sequencia            and
                   tt-ped-venda-aux.cdd-embarq  = i-embarque             and
                   tt-ped-venda-aux.nome-abrev  = tt-alocar.nome-abrev   and
                   tt-ped-venda-aux.nr-pedcli   = tt-alocar.nr-pedcli    and
                   tt-ped-venda-aux.ind-oper    = 1 NO-ERROR. /* Inclusao */
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-embarque,
       ...
       WHERE embarque.cdd-embarq = tt-embarque.cdd-embarq:
        ...
        IF dt-bas-dupl = ? THEN DO:
            /*Edson-18/07/2016 para replicar data base de duplicat par anota filha*/
            FIND FIRST fat-duplic WHERE
                  fat-duplic.nr-fatura   = nota-fiscal.nr-nota-fis   AND
                  fat-duplic.serie       = nota-fiscal.serie         AND 
                  fat-duplic.cod-estabel = nota-fiscal.cod-estabel  NO-LOCK NO-ERROR.

            IF AVAIL fat-duplic THEN
                 ASSIGN dt-bas-dupl = fat-duplic.dt-emissao.
            ELSE
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    IF TRUE THEN RETURN "ok".
    /*desativado temporariamento 22/02/2017 Ricardo para ver se para de dar erro de alocaá‰es*/
    FIND FIRST pesagem NO-LOCK
         WHERE pesagem.cdd-embarq = i-nr-emb-orig NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN  estab-log = nota-fiscal.cod-estabel
            serie-log = nota-fiscal.serie     
            nf-log    = nota-fiscal.nr-nota-fis.
    ...
    RUN cancelaNotaFiscal IN bo-cancela (INPUT nota-fiscal.cod-estabel,
                                         ...
                                         INPUT YES,
                                         INPUT YES,
                                         INPUT SESSION:TEMP-DIRECTORY + "cancela-ifbodi135.txt",
                                         INPUT c-seg-usuario).
    ...
    END.
    ELSE DO:
        /* grava log do erro de cancelamento*/
        ...
        IF FILE-INFO:PATHNAME <> ? THEN DO:
            OS-FILE = "m:\dts\log_prd\ifbodi135.LOG".
        END.
        ELSE DO:
           OS-FILE = "\\ungusb-vap01\Sistemas\DTS\Log_Prd".
           FILE-INFO:FILE-NAME = os-file.  
    
           os-file = "".
            IF FILE-INFO:PATHNAME <> ? THEN DO:
              OS-FILE = "\\ungusb-vap01\Sistemas\DTS\Log_Prd\ifbodi135.LOG".
            END.
        
        END.
        IF trim(os-file) <> "" THEN DO:
            ...
            OUTPUT TO VALUE(OS-FILE) APPEND.
             ...
             PUT UNFORMATTED  
                 ...
                 "Serie:" ";" serie-log ";"
                 "Estab:" ";"  estab-log ";"
                 "Emiss∆o:" ";" string(TODAY) ";" 
                 "Hora:" ";" string(TIME,"hh:mm:ss") ";"  
                 "Usu†rio:" ";" c-seg-usuario ";" .
        
                 
             FOR EACH RowErrors NO-LOCK:
                    PUT UNFORMATTED  "Nerro:" RowErrors.ErrorNumber ","
                                     "Tp:" RowErrors.ErrorType ","                            
                                     "Desc:" replace(RowErrors.ErrorDescription,chr(10)," ") ","
                                     "Help:" replace(RowErrors.ErrorHelp,CHR(10)," ") ",".
             END.
...
------------------------------------------------------------------------------*/
    ...
    /* CORRIGE PARA TODOS OS EMBARQUES
    ...
    IF AVAIL b-ped-venda THEN DO:
        ...
        IF CAN-FIND(FIRST b2-ped-item OF b-ped-venda
            ...
            assign tt-param-sppd309.destino        = 2
                   ...
                   tt-param-sppd309.de-cdd-embarq  = nota-fiscal.cdd-embarq
                   tt-param-sppd309.i-tipo         = 2
...
PROCEDURE pi-sppd309-ped :
/*------------------------------------------------------------------------------
  Purpose: Verifica alocaá∆o dos itens e se estiver errada executa programa de acerto
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    DEF INPUT PARAM p-nome-abrev AS CHARACTER NO-UNDO.
    DEF INPUT PARAM p-nr-pedcli  AS CHARACTER NO-UNDO.

    
    create tt-param-sppd309.
    assign tt-param-sppd309.destino        = 2
           tt-param-sppd309.usuario        = c-seg-usuario
           tt-param-sppd309.arquivo        = SESSION:TEMP-DIRECTORY + "sppd309a-ung-ped-" + STRING(nota-fiscal.cdd-embarq) + "-.tmp"
           tt-param-sppd309.data-exec      = today           
           tt-param-sppd309.c-nome-abrev   = p-nome-abrev
           tt-param-sppd309.c-nr-pedcli    = p-nr-pedcli  
         ...
         AND it-pre-fat.nr-pedcli  = tt-ped-venda-aux.nr-pedcli:    
            ...
            FOR EACH   tt-despreza
                 ...
                 FIND FIRST tt-aloc-man WHERE 
                     ...
                     tt-aloc-man.cod-localiz   =  tt-despreza.cod-localiz     and
                     tt-aloc-man.lote          =  tt-despreza.lote            and
                     tt-aloc-man.cod-refer     =  tt-despreza.cod-refer       and                                  
                     tt-aloc-man.nr-entrega    =  tt-despreza.nr-entrega      NO-ERROR.
                 IF NOT AVAIL tt-aloc-man THEN DO:
                     ...
                     ASSIGN  tt-aloc-man.cdd-embarq    =  tt-despreza.cdd-embarq    
                             ...
                             tt-aloc-man.cod-localiz   =  tt-despreza.cod-localiz   
                             tt-aloc-man.lote          =  tt-despreza.lote          
                             tt-aloc-man.cod-refer     =  tt-despreza.cod-refer     
                             tt-aloc-man.i-sequen      =  tt-despreza.i-sequen      
                             tt-aloc-man.nr-entrega    =  tt-despreza.nr-entrega  .  
                 END.
                 tt-aloc-man.qt-a-alocar = tt-aloc-man.qt-a-alocar + tt-despreza.qt-a-alocar.
