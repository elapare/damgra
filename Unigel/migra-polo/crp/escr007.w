...
define variable c-prog-gerado as character no-undo initial "escr007".
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Rela‡Æo de T¡tulos do Contas a Receber - escr007"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
run grapi/gr2003.p (input-output c-arquivo, output l-ok).
...
assign v-cod-prog-gerado = "escr007".


def var c-tit as char no-undo.

run grapi/gr2011.p (input v-cod-prog-gerado,
                    input "2.00.00.000",
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "escr007" + ".tmp".
...
run grapi/gr2007.p (input input frame f-pg-imp rs-destino, input input frame f-pg-imp c-arquivo).
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "crp/escr007rp.p".
