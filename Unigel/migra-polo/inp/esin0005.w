...
/*------------------------------------------------------------------------
File.............: esin0005.w
Description......: Controle de Projetos de Investimentos
Input Parameters : 
Output Parameters: 
...
define variable c-prog-gerado as character no-undo initial "esin0005".
...
define temp-table tt-param
    ...
    field i-num-projeto-ini      AS INT 
    field i-num-projeto-fim      AS INT 
    field i-num-ord-magnus-ini   AS INT  
    field i-num-ord-magnus-fim   AS INT  
    field tg-aberto              AS LOG 
    field tg-fechado-tec         AS LOG
    field tg-fechado-ctb         AS LOG
    field tg-cancelado           AS LOG
    field tg-congelado           AS LOG
...
def new shared var c-cod-estabel-ini      AS CHAR   FORMAT "x(3)"       initial "422"           NO-UNDO.  /*solic-318*/ 
...
def new shared var i-num-ord-magnus-fim   AS INT    FORMAT ">>>>>>>9"   INITIAL 99999999        NO-UNDO.    
...
DEFINE VARIABLE rs-opcao AS INTEGER INITIAL 1 
     ...
     RADIO-BUTTONS 
          "Resumido p/ Projeto", 1,
          "Completo p/ Projeto/ordem", 2
...
DEFINE VARIABLE tg-aberto AS LOGICAL INITIAL yes 
     LABEL "Aberto" 
     VIEW-AS TOGGLE-BOX
     SIZE 20 BY .83 TOOLTIP "Projeto em aberto" NO-UNDO.


DEFINE VARIABLE tg-fechado-tec AS LOGICAL INITIAL yes 
     LABEL "Fechado Tecnico" 
     VIEW-AS TOGGLE-BOX
     SIZE 20 BY .83 TOOLTIP "Projeto Fechado T‚cnico" NO-UNDO.


DEFINE VARIABLE tg-fechado-ctb AS LOGICAL INITIAL yes 
     LABEL "Fechado Cont bil" 
     VIEW-AS TOGGLE-BOX
     SIZE 20 BY .83 TOOLTIP "Projeto Fechado Cont bil" NO-UNDO.


DEFINE VARIABLE tg-cancelado AS LOGICAL INITIAL yes 
     LABEL "Cancelado" 
     VIEW-AS TOGGLE-BOX
     SIZE 20 BY .83 TOOLTIP "Projeto Cancelado" NO-UNDO.


DEFINE VARIABLE tg-congelado AS LOGICAL INITIAL yes 
     LABEL "Congelado" 
     VIEW-AS TOGGLE-BOX
     SIZE 20 BY .83 TOOLTIP "Projeto Congelado" NO-UNDO.
...
DEFINE FRAME f-pg-sel
   ...
   c-cod-estabel-ini label "Estabelecimento"
     at row 1 col 25 colon-aligned
     view-as fill-in 
     size 06 by .88
     font 1

   i-num-projeto-ini label "Nr.Projeto Invest."
     at row 2 col 25 colon-aligned
     view-as fill-in 
     size 15 by .88
     font 1

    i-num-projeto-fim no-label 
      at row 2 col 51 colon-aligned
      view-as fill-in 
      size 15 by .88
      font 1

    i-num-ord-magnus-ini label "Nr.Ordem no EMS"
      at row 3 col 25 colon-aligned
      view-as fill-in 
      size 15 by .88
      font 1

    i-num-ord-magnus-fim no-label 
      at row 3 col 51 colon-aligned
      view-as fill-in 
      size 15 by .88
      font 1

    dt-trans-ini label "Per¡odo Abertura Realizado"
      at row 4 col 25 colon-aligned
      view-as fill-in 
      size 15 by .88
      font 1

    dt-trans-fim NO-LABEL 
      at row 4 col 51 colon-aligned
      view-as fill-in 
      size 15 by .88
      font 1

    "Status do Projeto:" AT ROW 5.5 COL 10

    tg-aberto      AT ROW 6  COL 10
    tg-fechado-tec AT ROW 7  COL 10
    tg-fechado-ctb AT ROW 8  COL 10
    tg-cancelado   AT ROW 9  COL 10
    tg-congelado   AT ROW 10 COL 10

    rs-opcao   LABEL "Op‡äes:"    AT ROW 6  COL 40

    IMAGE-21 AT ROW 02.00 col 44
    IMAGE-22 AT ROW 02.00 col 48
    IMAGE-31 AT ROW 03.00 col 44
    IMAGE-32 AT ROW 03.00 col 48
    IMAGE-41 AT ROW 04.00 col 44
    IMAGE-42 AT ROW 04.00 col 48
    
    RECT-19 AT ROW 5.29 COL 2.14
    RECT-20 AT ROW 5.29 COL 40.14
...
CREATE WINDOW C-Win ASSIGN
   HIDDEN             = YES
   TITLE              = "Controle de Projetos de Investimentos - esin0005"
   HEIGHT             = 15
   WIDTH              = 81.14
   MAX-HEIGHT         = 22.35
   MAX-WIDTH          = 114.29
...
assign v-cod-prog-gerado = "esin0005".


def var c-tit as char no-undo.

run grapi/gr2013a.p (input v-cod-prog-gerado,
                    input "Controle de Projetos de Investimentos",
...
PROCEDURE enable_UI :
   ...
   DISPLAY  
       ...
       i-num-projeto-ini       
       i-num-projeto-fim       
       i-num-ord-magnus-ini  
       i-num-ord-magnus-fim 
       dt-trans-ini
       dt-trans-fim
       tg-aberto      
       tg-fechado-tec 
...
if return-value = "NOK" then do:
  ...
  return error.
   ...
   if  tt-param.destino = 2 then
       ...
       assign tt-param.arquivo = session:temp-directory + "esin0005" + ".tmp".

    assign tt-param.c-cod-estabel-ini      = input frame f-pg-sel c-cod-estabel-ini  
           tt-param.i-num-projeto-ini      = input frame f-pg-sel i-num-projeto-ini       
           tt-param.i-num-projeto-fim      = input frame f-pg-sel i-num-projeto-fim       
           tt-param.i-num-ord-magnus-ini   = input frame f-pg-sel i-num-ord-magnus-ini  
           tt-param.i-num-ord-magnus-fim   = input frame f-pg-sel i-num-ord-magnus-fim  
           tt-param.dt-trans-ini           = input frame f-pg-sel dt-trans-ini  
           tt-param.dt-trans-fim           = input frame f-pg-sel dt-trans-fim  
           tt-param.tg-aberto              = input frame f-pg-sel tg-aberto      
           tt-param.tg-fechado-tec         = input frame f-pg-sel tg-fechado-tec 
           tt-param.tg-fechado-ctb         = input frame f-pg-sel tg-fechado-ctb 
           tt-param.tg-cancelado           = input frame f-pg-sel tg-cancelado   
           tt-param.tg-congelado           = input frame f-pg-sel tg-congelado   
           tt-param.i-opcao                = input frame f-pg-sel rs-opcao   
.
...
if return-value = "NOK" then return error.
   ...
   if  session:set-wait-state("general") then.
    assign v-cod-prog-i-rprun = "inp/esin0005rp.p".
