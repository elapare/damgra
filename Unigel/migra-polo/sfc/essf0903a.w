...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "XX9999A"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    put "XX9999A" at 1 "9.99.99.999" at 69 today at 84 string(time,'HH:MM:SS':U) at 94 skip.
...
DEFINE VARIABLE h_essf0903v02 AS HANDLE NO-UNDO.
...
/* ************************  Frame Definitions  *********************** */
DEFINE FRAME f-cad
     bt-ok AT ROW 10.21 COL 3.14 HELP
          "Salva e sai"
     bt-save AT ROW 10.21 COL 14.14 HELP
          "Salva e cria novo registro"
     bt-cancela AT ROW 10.21 COL 25.14 HELP
          "Cancela"
     bt-ajuda AT ROW 10.21 COL 78.14
     rt-button AT ROW 10.04 COL 2
...
*/
    ...
    ASSIGN adm-object-hdl    =   w-incmdp.
...
THIS-PROCEDURE:ADM-DATA = 
     ...
     'TableIO-Target~`':U + /* Supported-Links attribute */
...
/* I-EPC009.I */
    ...
    DISABLE bt-ok bt-save bt-cancela bt-ajuda rt-button WITH FRAME f-cad.
...
/* I-EPC008 */
    ...
    ENABLE UNLESS-HIDDEN bt-ok bt-save bt-cancela bt-ajuda rt-button WITH FRAME f-cad.
...
/*--------------------------------------------------------------------------
    Library     : w-cadcom.i
    Purpose     : Permitir customizaªío para as window de cadastro complexo
    Syntax      : {include/w-cadcom.i}
    Description : Library utilizada para customizaªío da window de cadastro
                  complexo
    Author(s)   : Vanei
    Created     : 14/01/1997
    Notes       :
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    assign bt-save:sensitive in frame f-cad = no.
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
...
------------------------------------------------------------------------------*/
    if estado = "inclui":U then
        apply "choose":U to bt-save in frame f-cad.
    else
        apply "choose":U to bt-ok in frame f-cad.
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
PROCEDURE pi-entry :
/*------------------------------------------------------------------------------
...
OR ENDKEY OF w-incmdp ANYWHERE DO:
...
**
**    Funªío : LΩgica do bt-cancela para o cadastro de pai, do template w-incmdp.w
**
**    Data   : 26/09/1997
**
...
**
**    Funªío : LΩgica do bt-ok para o cadastro de pai, do template w-incmdp.w
**
**    Data   : 26/09/1997
**
**    Autor  : Sergio Weber Jr.
...
do  on error undo, return no-apply:
  RUN dispatch IN h_essf0903v02('update-record':U).
  IF return-value = "ADM-ERROR":U THEN
     RETURN NO-APPLY.
...
**
**    Funªío : LΩgica do bt-salva para o cadastro de pai, do template w-incmdp.w
**
**    Data   : 26/09/1997
**
**    Autor  : Sergio Weber Jr.
...
do  on error undo, return no-apply:
  ...
  if estado = 'copia':U then do:
     run pi-desabilita-chave in h_essf0903v02.
  end.  
  if estado = 'inclui':U then do:
     RUN dispatch IN wh-query('open-query':U).
     RUN dispatch IN h_essf0903v02 ('add-record':U).
  end.
...
IF VALID-HANDLE(w-incmdp) THEN DO:
    ASSIGN CURRENT-WINDOW                = w-incmdp 
       w-incmdp:KEEP-FRAME-Z-ORDER = YES
       THIS-PROCEDURE:CURRENT-WINDOW = w-incmdp.
  ...
  CASE adm-current-page: 
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'sfc/essf0903v02.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_essf0903v02 ).
       RUN set-position IN h_essf0903v02 ( 1.08 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 8.79 , 88.57 ) */
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'sfc/essf0903q01.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_essf0903q01 ).
       RUN set-position IN h_essf0903q01 ( 9.92 , 66.86 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */
       /* Links to SmartViewer h_essf0903v02. */
       RUN add-link IN adm-broker-hdl ( h_essf0903q01 , 'Record':U , h_essf0903v02 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_essf0903v02 ).
       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_essf0903v02 ,
...
------------------------------------------------------------------------------*/
  ENABLE bt-ok bt-save bt-cancela bt-ajuda rt-button 
...
**               igualando ambos
*************************************************************************/
if w-incmdp:width-chars < frame f-cad:width-chars then
    assign frame f-cad:width-chars = w-incmdp:width-chars.
else if frame f-cad:width-chars < w-incmdp:width-chars then
    assign w-incmdp:width-chars = frame f-cad:width-chars.
if w-incmdp:height-chars < frame f-cad:height-chars then
    assign frame f-cad:height-chars = w-incmdp:height-chars.
else if frame f-cad:height-chars < w-incmdp:height-chars then
    assign w-incmdp:height-chars = frame f-cad:height-chars.
assign w-incmdp:virtual-width-chars  = w-incmdp:width-chars
       w-incmdp:virtual-height-chars = w-incmdp:height-chars
       w-incmdp:min-width-chars      = w-incmdp:width-chars
       w-incmdp:max-width-chars      = w-incmdp:width-chars
       w-incmdp:min-height-chars     = w-incmdp:height-chars
       w-incmdp:max-height-chars     = w-incmdp:height-chars.
/* win-size.i */
...
def var c_nom_razao_social as char no-undo.
    ...
    assign c-programa-mg97 = caps("essf0903A")
    ...
    if  avail prog_dtsul then do:
       ...
       */
        ...
        assign c-titulo-prog-mg97    = caps(c-programa-mg97)
         ...
         assign w-incmdp:title = if l-achou-prog then
                                       c-titulo-prog-mg97
                                     + " - " 
                                     + c-programa-mg97 
                                     + " - " 
...
if index(replace(program-name(1),"~\","~/"),"go/g") = 0 then do:
  ...
  if  return-value = "2012" then do:
      ...
      if "w-incmdp" = "SmartDialog" or this-procedure:persistent = no then
        return "adm-error".
      else do:     
...
/* Transformacao Window *****************************************************/
    case i-template:
        when 2 then do: /* Cadastro Simples */
            ...
            run DisableMaxButton in h-prog(input w-incmdp:hWnd).
            delete procedure h-prog.
    
            assign w-incmdp:HIDDEN = yes
            w-incmdp:HIDDEN = no.
            apply "ENTRY":U to w-incmdp.
        end.
        ...
        when 9 then do: /* Inclui/Modifica Pai */
            /*** Ocultar MAX - MIN ***/
            run utp/ut-style.p persistent set h-prog.
            run DeleteMinMaxButtons in h-prog(input w-incmdp:hWnd).
            delete procedure h-prog.
        ...
        when 20 then do: /* Janela Detalhe */
            ...
            /*** Tornar Window Modal ***/
            assign h-pai           = SESSION:FIRST-CHILD.
            if  h-pai:handle = w-incmdp:handle then
                assign h-pai           = h-pai:NEXT-SIBLING
                       h-pai:SENSITIVE = no.
            else 
                assign  h-pai = w-incmdp:handle
                        h-pai = h-pai:parent.
            h-pai:sensitive = no.
...
/* ut9000.i */
  ...
  run pi-seta-acao in h_essf0903v02 (input estado).
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_essf0903q01 (INPUT v-row-table). 
