...
def temp-table tt-requis no-undo
    ...
    field cod-localiz-ini  as char    init "          "
    field cod-localiz-fim  as char    init "ZZZZZZZZZZ"
...
def temp-table tt-reporta no-undo
    ...
    FIELD tipo-rep      AS INT INIT 1 /*1 - Produá∆o, 2 - Refugo*/
    FIELD nr-ord-orig   LIKE ord-prod.nr-ord-produ
...
DEFINE VARIABLE text-peso AS CHARACTER FORMAT "X(95)" INITIAL ""
VIEW-AS TEXT 
SIZE 73.57 BY .62 NO-UNDO
fgcolor 5
font 6. 
...
DEFINE FRAME f-relat
    ...
    d-peso-liquido LABEL "Peso L°quido"
     ...
     help "Aperte a tecla de pesagem no m¢dulo da balanáa."
     at row 19.7 col 14 colon-aligned
     view-as fill-in 
     size 15 by .88
     font 1

     text-peso no-label  at row 19.7 col 35
...
DO:
    ...
    IF (c-it-codigo-cons:screen-value in frame f-relat <> "" AND  c-emenda-1:SCREEN-VALUE IN FRAME f-relat <> "" AND
       c-it-codigo-cons:screen-value in frame f-relat <> c-it-codigo-emen1:SCREEN-VALUE IN FRAME f-relat ) OR
       ...
       c-it-codigo-emen1:screen-value in frame f-relat <> c-it-codigo-emen2:SCREEN-VALUE IN FRAME f-relat )   THEN DO:


         run utp/ut-msgs.p (input "show",
                               input 17006,
                               input "Item do Lote Incompativel").
    ...
    IF  ( (INDEX(c-it-codigo-cons:SCREEN-VALUE IN FRAME f-relat,"TSB") > 0 OR 
       INDEX(c-it-codigo-cons:SCREEN-VALUE IN FRAME f-relat,"TBB") > 0 OR
       INDEX(c-it-codigo-cons:SCREEN-VALUE IN FRAME f-relat,"TBH") > 0) AND 
       INDEX(c-it-codigo-cons:SCREEN-VALUE IN FRAME f-relat,"PROC") > 0 ) AND 
      RETURN-VALUE = "nok" THEN
           run utp/ut-msgs.p (input "show",
                          input 17006,
                          input "Lote ainda n∆o liberado~~Verifique a informaá∆o data e hora de liberaá∆o na etiqueta do lote " ).
...
DO:
    ...
    run CalculaQuantSaldo.
    IF  ( (INDEX(c-it-codigo-emen1:SCREEN-VALUE IN FRAME f-relat,"TSB") > 0 OR 
       INDEX(c-it-codigo-emen1:SCREEN-VALUE IN FRAME f-relat,"TBB") > 0 OR
       INDEX(c-it-codigo-emen1:SCREEN-VALUE IN FRAME f-relat,"TBH") > 0) AND 
       INDEX(c-it-codigo-emen1:SCREEN-VALUE IN FRAME f-relat,"PROC") > 0 ) AND 
      RETURN-VALUE = "nok" THEN
           run utp/ut-msgs.p (input "show",
                          input 17006,
                          input "Lote ainda n∆o liberado~~Verifique a informaá∆o data e hora de liberaá∆o na etiqueta do lote ").
...
DO:
    ...
    run CalculaQuantSaldo.
    IF  ( (INDEX(c-it-codigo-emen2:SCREEN-VALUE IN FRAME f-relat,"TSB") > 0 OR 
      INDEX(c-it-codigo-emen2:SCREEN-VALUE IN FRAME f-relat,"TBB") > 0 OR
      INDEX(c-it-codigo-emen2:SCREEN-VALUE IN FRAME f-relat,"TBH") > 0) AND 
      INDEX(c-it-codigo-emen2:SCREEN-VALUE IN FRAME f-relat,"PROC") > 0 ) AND 
     RETURN-VALUE = "nok" THEN
          run utp/ut-msgs.p (input "show",
                         input 17006,
                         input "Lote ainda n∆o liberado~~Verifique a informaá∆o data e hora de liberaá∆o na etiqueta do lote " ).
...
on LEAVE OF i-nr-ord-produ in frame f-relat do:
    ...
    IF AVAIL ord-prod THEN DO:
      ...
      IF INPUT FRAME f-relat rs-tipo = 1 THEN
        ...
        IF ord-prod.nr-ord-produ > 70000000 AND  
           ord-prod.nr-ord-produ < 80000000 THEN DO:
            ASSIGN i-largura:SENSITIVE IN FRAME f-relat= no
                   i-diin:Sensitive IN FRAME f-relat = NO.
...
PROCEDURE CalculaQuantSaldo :
    ...
    IF c-lote-cons:screen-value in frame f-relat <> "" THEN DO:
        ...
        for each saldo-estoq fields (qtidade-atu qt-alocada qt-aloc-ped it-codigo cod-depos cod-refer cod-estabel cod-localiz lote
            ...
            RUN pi-valida-proc (INPUT saldo-estoq.it-codigo , INPUT  saldo-estoq.lote ).
...
------------------------------------------------------------------------------*/
      ...
      IF  AVAIL ped-venda THEN  DO:
          ...
          FIND FIRST espmulti.if-ped-venda WHERE if-ped-venda.nr-pedido = ped-venda.nr-pedido NO-LOCK NO-ERROR.

          IF AVAIL if-ped-venda THEN FIND FIRST ped-venda WHERE ped-venda.nr-pedido = if-ped-venda.nr-pedido-relac NO-LOCK NO-ERROR.
      
         IF  AVAIL ped-venda THEN RUN pdp\upc\upc-pd4000-a.p (INPUT b-ord-prod.cod-estabel,INPUT b-ord-prod.it-codigo, INPUT ped-venda.nome-abrev).
      END.
...
PROCEDURE criaTT:
    ...
    FIND first ord-prod no-lock 
        WHERE ord-prod.nr-ord-produ = int(i-nr-ord-produ:SCREEN-VALUE IN FRAME f-relat) NO-ERROR.
    ...
    ASSIGN tt-reporta.nr-ord-produ = ord-prod.nr-ord-produ
           ...
           tt-reporta.nr-ord-orig  = tt-estacao.nr-ord-produ.
    ...
    if index(tt-reporta.it-codigo,"OG" ) > 0 OR INPUT FRAME f-relat rs-opcao = 2 then do:
        IF ord-prod.cod-item-refugo <> "" THEN DO:
            find item where item.it-codigo =  ord-prod.cod-item-refugo no-lock no-error.
        
            if (avail item and item.ge-codigo = 47) OR INPUT FRAME f-relat rs-opcao = 2 then 
                ASSIGN tt-reporta.cod-localiz = ITEM.cod-localiz
                       tt-reporta.cod-depos   = item.deposito-pad
                       tt-reporta.tipo-rep    = 2. /*Refugo*/
        END.
        ELSE DO:
            find item where item.it-codigo =  tt-reporta.it-codigo no-lock no-error.
            if (avail item and item.ge-codigo = 47) OR INPUT FRAME f-relat rs-opcao = 2 then 
                ASSIGN tt-reporta.cod-localiz = item.inform-compl
    ...
    do with frame f-relat :
        ...
        ELSE DO:
            ...
            for each tt-reporta
                ...
                END.
                ELSE ASSIGN c-nr-pallet = string(ord-prod.nr-ord-produ).
                IF LENGTH(c-nr-pallet) > 14 THEN
                    run criaErro (17006,"Numeraá∆o do Pallet excede 14 posiá‰es!").
            end.
  ...
  if lErro = false then do:
    ...
    for each tt-reporta
        break by tt-reporta.nr-ord-produ:
            ...
            FOR EACH  reservas 
                WHERE reservas.nr-ord-produ = tt-reporta.nr-ord-produ:
                ASSIGN reservas.estado   = 2. 
            END.
            
            FOR EACH tt-lote-cons NO-LOCK.

              FOR EACH  reservas EXCLUSIVE-LOC
                 WHERE reservas.nr-ord-produ = tt-reporta.nr-ord-produ   
                   AND reservas.it-codigo    = tt-lote-cons.it-codigo 
                   AND reservas.cod-refer    = tt-lote-cons.cod-refer:
                  ASSIGN reservas.estado     = 1
                         reservas.lote-serie = "".
              END.
              ...
              FIND FIRST reservas 
                 WHERE reservas.nr-ord-produ = tt-reporta.nr-ord-produ   
                   AND reservas.it-codigo    = tt-lote-cons.it-codigo  
                   AND reservas.cod-refer    = tt-lote-cons.cod-refer  
                   AND reservas.estado       = 1 NO-LOCK NO-ERROR.
                   
              IF NOT AVAIL reservas  THEN DO:
                 FIND FIRST  reservas 
                    WHERE reservas.nr-ord-produ = tt-reporta.nr-ord-produ   
                      AND reservas.it-codigo    = tt-lote-cons.it-codigo 
                      AND trim(reservas.cod-refer) <> "" NO-LOCK NO-ERROR.
        ...
        for each tt-lote-reporte where
           ...
           IF param-global.modulo-cq and 
            ...
            if temp-table tt-erro:has-records then do:
   			 ...
   			 where cadast_msg.cdn_msg = tt-erro.cd-erro no-error.
            ...
            find last ord-rep no-lock
                where ord-rep.nr-ord-produ = ord-prod.nr-ord-produ no-error.    
            ...
            */
            IF lin-prod.sum-requis = 2 THEN DO:
                ...
                FOR EACH movto-estoq EXCLUSIVE-LOCK USE-INDEX operacao
                    ...
                    ASSIGN movto-estoq.nr-reporte = ord-rep.nr-reporte.

                    IF AVAIL movto-mat THEN
                        ASSIGN movto-mat.nr-reporte = ord-rep.nr-reporte.
                END.
            END.
            ...
            if avail tt-rep-oper-ctrab then do:
               ...
               IF ord-prod.cod-item-refugo <> "" and rep-oper-ctrab.qtd-operac-refgda = rep-oper-ctrab.qtd-operac-reptda THEN DO:
               END.
               ELSE DO:
                    find last movto-mat
                    ...
                    if avail movto-mat then do:
                       ...
                       IF NOT(avail item and
                          item.cd-folh-lote <> "" AND movto-mat.lote <> "RECICL" AND movto-mat.lote <> "") THEN  RUN pi-grava-erro-log-ctl (INPUT 
                                                                                string(TODAY) + "-" + string(TIME,"hh:mm:ss") + "-" + 
                                                                                " Logado: " +  string(c-seg-usuario) +
                                                                                "cd-folh-lote Ordem:" + 
                                                                                STRING(tt-lote-reporte.nr-ord-produ) + " - Item:" + 
                       ...
                       if avail item and
                          item.cd-folh-lote <> "" AND movto-mat.lote <> "RECICL" AND movto-mat.lote <> "" then do:
    ...
    if lErro then do:
        ...
        for each tt-erro:
           ...
           if avail cadast_msg and 
              cadast_msg.idi_tip_msg > 1 then do:
              create tt-erro2.
...
if AVAIL  tt-lote-reporte AND tt-lote-reporte.lote <> "RECICL" and tt-lote-reporte.lote <> "" then do:
   run utp/ut-msgs.p (input "show",
                      input 27100,
...
------------------------------------------------------------------------------*/
    ...
    FIND LAST oper-ord NO-LOCK
        WHERE oper-ord.nr-ord-produ     = ord-prod.nr-ord-produ 
          AND oper-ord.log-operac-final = YES NO-ERROR.
    
    create tt-rep-prod.
    assign tt-rep-prod.tipo                  = ord-prod.rep-prod
           ...
           tt-rep-prod.qt-reporte            = IF tt-reporta.tipo-rep = 2 THEN 0 ELSE tt-lote-reporte.quantidade
           tt-rep-prod.qt-refugo             = IF tt-reporta.tipo-rep = 1 THEN 0 ELSE tt-lote-reporte.quantidade
           tt-rep-prod.cod-refer             = ord-prod.cod-refer
           ...
           tt-rep-prod.dep-refugo            = IF tt-reporta.tipo-rep = 1 THEN substring(param-cp.char-2,1,3) ELSE tt-reporta.cod-depos
           tt-rep-prod.loc-refugo            = IF tt-reporta.tipo-rep = 1 THEN cInformCompl                   ELSE tt-reporta.cod-localiz
    ...
    /*OFF GRADE - Rastreabilidade da Ordem Origem
      OFF SPEC  - Rastreabilidade da Ordem Origem*/
    IF ord-prod.it-codigo MATCHES "*OG" OR
       ord-prod.it-codigo MATCHES "*OS" THEN
        ASSIGN tt-rep-prod.nro-docto = string(tt-reporta.nr-ord-orig).
...
assign tt-rep-oper-ctrab.cod-ferr-prod         = ""
       ...
       tt-rep-oper-ctrab.qtd-operac-refgda     = IF tt-reporta.tipo-rep = 1 THEN 0          ELSE deQuantNec
       tt-rep-oper-ctrab.qtd-operac-aprov      = IF tt-reporta.tipo-rep = 1 THEN deQuantNec ELSE 0
...
for each tt-reporta
    ...
    ELSE DO:
        ...
        ASSIGN c-nr-pallet = "X" + STRING(tt-reporta.nr-ord-produ).
...
IF i-nr-ped <> 0 THEN DO:
    ...
    FOR each pallet
        WHERE pallet.nr-pedido = i-nr-ped
          ...
          AND ENTRY(1,pallet.nr-pallet,"/") = c-nr-pallet USE-INDEX pedido NO-LOCK :
    
        ASSIGN i-seq-ult-pallet = DEC(entry(2,pallet.nr-pallet,"/")) NO-ERROR. 
...
IF r-pallet = ? THEN DO:
       ...
       ASSIGN i-seq-ult-pallet = 0
              c-nr-pallet      = TRIM(c-nr-pallet).

       IF  LENGTH(TRIM(c-nr-pallet)) > 14 THEN DO:
                        
             c-nr-pallet = SUBSTRING(c-nr-pallet,1,14) .
       ...
       REPEAT:
            ...
            IF i-seq-ult-pallet > 2000 THEN DO:
            ...
            FIND FIRST PALLET WHERE PALLET.NR-PALLET = c-nr-pallet + "/" + TRIM(STRING(i-seq-ult-pallet,">>99")) NO-LOCK NO-ERROR.
            IF NOT AVAIL PALLET THEN DO:
                     
                   c-nr-pallet = c-nr-pallet + "/" + TRIM(STRING(i-seq-ult-pallet,">>99")).
                   
                   LEAVE.
            END.
            ELSE DO:
                IF  pallet.nr-bobina < de-qt-palete AND
                    pallet.cod-refer = ord-prod.cod-refer AND
                    pallet.it-codigo = ord-prod.it-codigo AND
                    pallet.situacao = 1 THEN DO:
                    ASSIGN i-seq-pallet = DEC(entry(2,pallet.nr-pallet,"/")).
...
/*
ELSE ASSIGN  c-nr-pallet = c-nr-pallet + "/" + TRIM(STRING(i-seq-pallet,">99")).
...
do  on error undo, return error
 ...
 i-ct = 0.
    ...
    text-peso:screen-value IN FRAME f-relat = "***Aperte a tecla de pesagem no m¢dulo da balanáa..:".
    ...
    */
    ...
    DO WHILE TRUE:              
         ...
         IF text-peso:screen-value IN FRAME f-relat <> "***Aperte a tecla de pesagem no m¢dulo da balanáa..:" + string(16 - (I-TIME-FIM - I-TIME-INI),"-99") THEN 
          text-peso:screen-value IN FRAME f-relat = "***Aperte a tecla de pesagem no m¢dulo da balanáa..:" + string(16 - (I-TIME-FIM - I-TIME-INI),"-99").

         IF (I-TIME-FIM - I-TIME-INI) > 15 THEN LEAVE.
    ...
    d-peso = dec(c-peso) no-error.
    
        text-peso:screen-value IN FRAME f-relat = "***Peso n∆o capturado, por favor digite o peso manualmente ou saia e entre no campo.".
...
PROCEDURE pi-valida-proc.
DEFINE INPUT param   p-it-codigo AS CHAR   NO-UNDO.
DEFINE INPUT param   p-lote      AS CHAR   NO-UNDO.

DEFINE VARIABLE d-dt-trans AS DATE      NO-UNDO.
DEFINE VARIABLE c-hr-trans AS CHAR      NO-UNDO.
DEFINE VARIABLE i-hr-trans AS INTEGER     NO-UNDO.

    IF (INDEX(p-it-codigo,"TSB") > 0 OR 
       INDEX(p-it-codigo,"TBB") > 0 OR
       INDEX(p-it-codigo,"TBH") > 0) AND INDEX(p-it-codigo,"PROC") > 0   THEN DO:
    
        IF CAN-FIND (FIRST  ext_usuar_grp_usuar WHERE ext_usuar_grp_usuar.cod_grp_usuar = "METPROC" /* AND
                             ext_usuar_grp_usuar.cod_usuario c-seg-usuario*/  )
            THEN RETURN "ok".

        FIND FIRST lote-prod WHERE 
            lote-prod.it-codigo = p-it-codigo AND
            lote-prod.lote = p-lote NO-LOCK NO-ERROR.
    
        IF AVAIL lote-prod THEN DO:
            FIND LAST movto-estoq WHERE 
               movto-estoq.nr-ord-produ = lote-prod.nr-ord-produ AND
               movto-estoq.lote = p-lote AND
               movto-estoq.esp-docto = 1 USE-INDEX ord-seq NO-LOCK NO-ERROR.
           
            IF AVAIL movto-estoq THEN DO:
                d-dt-trans = movto-estoq.dt-trans.
                c-hr-trans = movto-estoq.hr-trans.
                run pi-formatted-time-to-sec (input  replace(c-hr-trans,":",""),
                                          output i-hr-trans).

                IF TODAY < d-dt-trans + 1 THEN do:
                    l-proc-ok = NO.
                    RETURN "nok".
                END.
                IF d-dt-trans + 1  = TODAY AND TIME < i-hr-trans THEN DO:
                    l-proc-ok = NO.
                    RETURN "nok".
                END.
