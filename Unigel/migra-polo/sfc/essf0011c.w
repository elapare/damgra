...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "essf0011C"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    put "essf0011C" at 1 "2.00.00.002" at 69 today at 84 string(time,'HH:MM:SS':U) at 94 skip.
...
DEF INPUT-OUTPUT PARAM  p-cod-estabel    LIKE estabelec.cod-estabel.                           
DEF INPUT-OUTPUT PARAM  p-nr-pedido-ini  LIKE ped-venda.nr-pedido.
DEF INPUT-OUTPUT PARAM  p-nr-pedido-fim  LIKE ped-venda.nr-pedido.
DEF INPUT-OUTPUT PARAM  p-nome-ini       LIKE ped-venda.nome-abrev.
DEF INPUT-OUTPUT PARAM  p-nome-fim       LIKE ped-venda.nome-abrev.
DEF INPUT-OUTPUT PARAM  p-tp-ped-ini     LIKE ped-venda.tp-ped.
DEF INPUT-OUTPUT PARAM  p-tp-ped-fim     LIKE ped-venda.tp-ped.
DEF INPUT-OUTPUT PARAM  p-data-ini       AS DATE FORMAT "99/99/9999".
DEF INPUT-OUTPUT PARAM  p-data-fim       AS DATE FORMAT "99/99/9999".
DEF INPUT-OUTPUT PARAM  p-i-mercado      AS INTEGER.
DEF OUTPUT       PARAM  i-acao           AS INTEGER NO-UNDO.

/* _UIB-CODE-BLOCK-END */

assign p-i-mercado = 3
       p-data-fim = p-data-fim + 180.
...
DEFINE VARIABLE c-cod-estabel AS CHARACTER FORMAT "X(03)":U INITIAL "422" 
...
DEFINE VARIABLE c-tp-ped-fim AS CHARACTER FORMAT "X(2)":U INITIAL "ZZ" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 NO-UNDO.

DEFINE VARIABLE c-tp-ped-ini AS CHARACTER FORMAT "X(2)":U 
     LABEL "Tipo Pedido" 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 NO-UNDO.
...
DEFINE FRAME F-Main
     c-cod-estabel AT ROW 1.17 COL 16 COLON-ALIGNED HELP
          "Estabelecimento"
     c-desc-estab AT ROW 1.17 COL 22 COLON-ALIGNED NO-LABEL
     nr-pedido-ini AT ROW 2.17 COL 16 COLON-ALIGNED HELP
          "N£mero Pedido Inicial"
     nr-pedido-fim AT ROW 2.17 COL 49 COLON-ALIGNED HELP
          "N£mero Pedido Final" NO-LABEL
     c-emitente-ini AT ROW 3.17 COL 16 COLON-ALIGNED HELP
          "Cliente Inicial"
     c-emitente-fim AT ROW 3.17 COL 49 COLON-ALIGNED HELP
          "Cliente Final" NO-LABEL
     c-tp-ped-ini AT ROW 4.17 COL 16 COLON-ALIGNED
     c-tp-ped-fim AT ROW 4.17 COL 49 COLON-ALIGNED NO-LABEL
     dt-entrega-ini AT ROW 5.17 COL 16 COLON-ALIGNED HELP
          "Data de Entrega Inicial"
     dt-entrega-fim AT ROW 5.17 COL 49 COLON-ALIGNED HELP
          "Date de Entrega Final" NO-LABEL
     rs-mercado AT ROW 7.13 COL 19 HELP
          "Mercado" NO-LABEL
     bt-ok AT ROW 9.25 COL 2.86
     bt-cancelar AT ROW 9.25 COL 13.86
     bt-ajuda AT ROW 9.29 COL 56.14
     IMAGE-11 AT ROW 5.17 COL 32.29
     IMAGE-12 AT ROW 5.17 COL 46.14
     IMAGE-13 AT ROW 4.13 COL 32.29
     IMAGE-14 AT ROW 4.13 COL 46.14
     IMAGE-3 AT ROW 2.17 COL 32.29
     IMAGE-4 AT ROW 2.17 COL 46.14
     IMAGE-5 AT ROW 3.13 COL 32.29
     IMAGE-6 AT ROW 3.13 COL 46.14
     RECT-1 AT ROW 9.04 COL 1.86
     RECT-14 AT ROW 6.54 COL 2.86
     RECT-23 AT ROW 1 COL 1
     "Mercado" VIEW-AS TEXT
          SIZE 8.14 BY .67 AT ROW 6.21 COL 4.72
...
/* I-EPC009.I */
    ...
    DISABLE nr-pedido-ini nr-pedido-fim c-emitente-ini c-emitente-fim c-tp-ped-ini c-tp-ped-fim dt-entrega-ini dt-entrega-fim rs-mercado bt-ok bt-cancelar bt-ajuda IMAGE-11 IMAGE-12 IMAGE-13 IMAGE-14 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 RECT-1 RECT-14 RECT-23 WITH FRAME F-Main.
...
/* I-EPC008 */
    ...
    ENABLE UNLESS-HIDDEN nr-pedido-ini nr-pedido-fim c-emitente-ini c-emitente-fim c-tp-ped-ini c-tp-ped-fim dt-entrega-ini dt-entrega-fim rs-mercado bt-ok bt-cancelar bt-ajuda IMAGE-11 IMAGE-12 IMAGE-13 IMAGE-14 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 RECT-1 RECT-14 RECT-23 WITH FRAME F-Main.
...
ON END-ERROR OF w-window /* ParÉmetros - essf0011C */
...
ON WINDOW-CLOSE OF w-window /* ParÉmetros - essf0011C */
DO:
...
DO:
  ...
  ASSIGN p-cod-estabel   = INPUT FRAME F-Main c-cod-estabel
         p-nr-pedido-ini = INPUT FRAME F-Main nr-pedido-ini
         p-nr-pedido-fim = INPUT FRAME F-Main nr-pedido-fim 
         p-nome-ini      = INPUT FRAME F-Main c-emitente-ini
         p-nome-fim      = INPUT FRAME F-Main c-emitente-fim
         p-tp-ped-ini    = INPUT FRAME F-Main c-tp-ped-ini
         p-tp-ped-fim    = INPUT FRAME F-Main c-tp-ped-fim
         p-data-ini      = INPUT FRAME F-Main dt-entrega-ini
         p-data-fim      = INPUT FRAME F-Main dt-entrega-fim 
         p-i-mercado     = INPUT FRAME F-Main rs-mercado.
...
------------------------------------------------------------------------------*/
  DISPLAY c-cod-estabel c-desc-estab nr-pedido-ini nr-pedido-fim c-emitente-ini 
          c-emitente-fim c-tp-ped-ini c-tp-ped-fim dt-entrega-ini dt-entrega-fim 
          rs-mercado 
      WITH FRAME F-Main IN WINDOW w-window.
  ENABLE c-cod-estabel nr-pedido-ini nr-pedido-fim c-emitente-ini c-emitente-fim c-tp-ped-ini 
         c-tp-ped-fim dt-entrega-ini dt-entrega-fim rs-mercado bt-ok 
         bt-cancelar bt-ajuda IMAGE-11 IMAGE-12 IMAGE-13 IMAGE-14 IMAGE-3 
         IMAGE-4 IMAGE-5 IMAGE-6 RECT-1 RECT-14 RECT-23 
      WITH FRAME F-Main IN WINDOW w-window.
...
def var c_nom_razao_social as char no-undo.
    ...
    assign c-programa-mg97 = caps("essf0011C")
...
/* ut9000.i */
  ...
  ASSIGN c-cod-estabel  = p-cod-estabel
         nr-pedido-ini  = p-nr-pedido-ini
         nr-pedido-fim  = p-nr-pedido-fim
         c-emitente-ini = p-nome-ini     
         c-emitente-fim = p-nome-fim     
         c-tp-ped-ini   = p-tp-ped-ini   
         c-tp-ped-fim   = p-tp-ped-fim   
         dt-entrega-ini = p-data-ini     
         dt-entrega-fim = p-data-fim     
         rs-mercado     = p-i-mercado.
