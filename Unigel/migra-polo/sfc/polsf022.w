...
define variable c-prog-gerado as character no-undo initial "polsf022".
...
def new global shared var v_cod_usuar_corren   like mgmulti.usuar_mestre.cod_usuario no-undo.
...
/****************************************************************************
**
**   Include: CPAPI001.I  - Definiá∆o das Temp-Tables do Reporte
**
*****************************************************************************/
...
IF INPUT FRAME f-relat rs-tipo = 1 THEN return no-apply.
  ...
  FIND FIRST item where
    ...
    RUN polo/sfc/polsf024.w (INPUT-OUTPUT table tt-defeito,
                             INPUT tp-defeito,
                             INPUT nr-bobina-def).
...
DO:
    ...
    ASSIGN c-arquivo-jr = search(c-temp-jr + "polsf023.txt"). 
...
if return-value = 'nok' then
    ...
    /*OUTPUT TO "v:\temp\polsf022.txt" APPEND.
...
c-temp-jr = session:temp-directory.
        ...
        OUTPUT TO value(c-temp-jr + "polsf023.txt").
...
assign v-cod-prog-gerado = "polsf022".



def var c-tit as char no-undo.

ASSIGN c-tit = "polsf022 - Reporte de Produá∆o no Corte/Recorte/Metalizaá∆o".
...
------------------------------------------------------------------------------*/
    ...
    do with frame f-relat :
        ...
        ELSE DO:
            ...
            if input dt-trans-fim > pol-param-estab.data-corte THEN 
                run criaErro (25997,"Transaá∆o n∆o permitida!Verifique Data de Corte informada nos ParÉmetros Estabelecimento - POLSF008.").
            ...
            for each tt-reporta
                ...
                END.
                ELSE ASSIGN c-nr-pallet = substring(string(ord-prod.nr-ord-produ,"99999999"),1,1) + substring(string(ord-prod.nr-ord-produ,"99999999"),4,5).
    ...
    if temp-table tt-erro:has-records then do:
     ...
     for each tt-reporta where tt-reporta.cod-depos = "cq" no-lock:
           if search("m:\dts\LOG_prd\polsf022.cq") <> ?  then do:

                OUTPUT TO m:\dts\LOG_prd\polsf022.cq APPEND.
                for each tt-lote-reporte where tt-lote-reporte.nr-ord-produ = tt-reporta.nr-ord-produ:
       

                    put "Prog: polsf022-APOS" FORMAT "x(19)" string(TODAY) "-" 
...
for each tt-reporta:
    for each tt-lote-reporte where
        ...
        IF l-peso-bal THEN DO:
          if search("m:\dts\LOG_prd\polsf022.LOG") <> ? then do:
                OUTPUT TO m:\dts\LOG_prd\polsf022.LOG APPEND.

                    DISP "Prog: polsf002 " FORMAT "x(19)" string(TODAY) "-" 
                         string(TIME,"hh:mm:ss") "-" 
                         " Logado: " string(c-seg-usuario) SKIP 
                         WITH WIDTH 300.
        ...
        IF tt-reporta.cod-depos = "cq" THEN DO:
           if search("m:\dts\LOG_prd\polsf022.cq") <> ?  then do:
                OUTPUT TO m:\dts\LOG_prd\polsf022.cq APPEND.
                    ...
                    put "Prog: polsf022-ANTES" FORMAT "x(19)" string(TODAY) "-" 
                         string(TIME,"hh:mm:ss") "-" 
                         " Logado: " string(c-seg-usuario) SKIP 
                        .
    ...
    for each tt-reporta
        ...
        for each tt-lote-reporte where
           ...
           IF param-global.modulo-cq and 
            ...
            IF tt-rep-prod.cod-depos = "cq" THEN DO:
                if search("m:\dts\LOG_prd\polsf022.cq") <> ?  then do:
                            ...
                            OUTPUT TO m:\dts\LOG_prd\polsf022.cq APPEND.
                                ...
                                put "Prog: polsf022-durante" FORMAT "x(19)" string(TODAY) "-" 
                                     string(TIME,"hh:mm:ss") "-" 
                                     " Logado: " string(c-seg-usuario) SKIP 
                                    .
            ...
            RUN polo/sfc/polsf002-esp.p (INPUT nr-ord-produ-jr-pro,
            ...
            case lin-prod.sum-requis:
                when 1 then do:
                    ...
                    run polo/sfc/polsf002c.p (input-output table tt-param,
                                              input        tt-rep-prod.qt-reporte).
                end.
                when 2 then do:
                    assign tt-param.h-handle = hRequis.
                    run polo/sfc/polsf002b.p (input-output table tt-param,
...
if AVAIL  tt-lote-reporte AND tt-lote-reporte.lote <> "RECICL" then do:
   ...
   if return-value = "yes" then do:
      run pi-etiqueta.
      /*run svam\ctlot\ctl001p.w (input table tt-digita).*/
   end.
end.
...
for each tt-reporta
    ...
    ELSE DO:
        ...
        ASSIGN c-nr-pallet = "X" + substring(string(tt-reporta.nr-ord-produ,"99999999"),1,1) + substring(string(tt-reporta.nr-ord-produ,"99999999"),4,5).
...
IF r-pallet = ? THEN DO:
    ...
    ASSIGN i-seq-ult-pallet = 0
        ...
        IF  LENGTH(TRIM(c-nr-pallet)) > 7 THEN DO:
            
            
             c-nr-pallet = SUBSTRING(c-nr-pallet,1,7) .
       ...
       REPEAT:
            ...
            IF i-seq-ult-pallet >= 100 AND LENGTH(TRIM(c-nr-pallet)) > 6 THEN DO:

                c-nr-pallet = SUBSTRING(c-nr-pallet,1,2) + SUBSTRING(c-nr-pallet,4,4).                                                
            
            END.

            IF i-seq-ult-pallet > 1000 THEN DO:
    ...
    IF  r-pallet = ? THEN DO:
       ...
       IF i-seq-ult-pallet >= 100  THEN DO:
                ...
                MESSAGE
                    "Qtd. paletes passou de 100, ser† criado normalmente o palete com numeraá∆oo com menos uma posiá∆o no n£mero da ordem de produá∆o"
                       + CHR(10) + "N£mero assumido:" + c-nr-pallet  VIEW-AS ALERT-BOX.
...
run svam/ctlot/ctl001prp.p (input raw-param,
...
PROCEDURE pi-grava-erro-log-ctl.
    ...
    OUTPUT TO m:\dts\LOG_prd\polsf022-erro.LOG APPEND.
