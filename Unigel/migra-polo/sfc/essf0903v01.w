...
************************************************************************/
/* validaá∆o de quantidade de emendas no pedido - Edson - Amgra - 06/12/07 */
...
if  c-arquivo-log <> "" and c-arquivo-log <> ? then do:
    find prog_dtsul
        where prog_dtsul.cod_prog_dtsul = "V99XX999"
        no-lock no-error.
   ...
   if not avail prog_dtsul then do:
    ...
    put "V99XX999" at 1 "9.99.99.999" at 69 today at 84 string(time,'HH:MM:SS':U) at 94 skip.
...
DEFINE VARIABLE i-empresa AS char    NO-UNDO.
DEFINE VARIABLE c-conta-contabil AS CHARACTER FORMAT "x(17)" LABEL "Conta Cont†bil"
     VIEW-AS FILL-IN 
     SIZE 17 BY .88 NO-UNDO.
def var data-medio as date no-undo.
DEFINE BUFFER b-item FOR ITEM.
def var h_p-cadsi2 as handle no-undo.
...
/* ************************  Frame Definitions  *********************** */
DEFINE FRAME f-main
     pallet.cod-estabel AT ROW 1.17 COL 22 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .88
     pallet.it-codigo AT ROW 2.17 COL 22 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 17.14 BY .88
     pallet.data-pallet AT ROW 2.17 COL 66.72 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     pallet.nr-pallet AT ROW 3.17 COL 22 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 14.14 BY .88
     pallet.cod-localiz AT ROW 3.17 COL 36.57 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 14.14 BY .88
     fi-situacao AT ROW 3.17 COL 66.72 COLON-ALIGNED
...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "l:\servicos\especificos\polo\programas\polo\sfc\essf0903v01.w should only be RUN PERSISTENT.":U
...
/* -----------------------------------------------------------
    ...
    DISABLE rt-key WITH FRAME f-main.
...
/* -----------------------------------------------------------
   ...
   /* EPC Before Enable do Container */
    ...
    ENABLE UNLESS-HIDDEN rt-key WITH FRAME f-main.
...
/* I-EPC006.I */
  ...
  ************************************************************/ 
      ...
      IF AVAILABLE pallet THEN
          DISPLAY UNLESS-HIDDEN pallet.cod-estabel pallet.it-codigo pallet.data-pallet pallet.nr-pallet pallet.cod-localiz fi-situacao
...
/************ Customizacao Datasul S.A. - John Cleber Jaraceski - 29/03/2000
             ...
             DISPLAY UNLESS-HIDDEN pallet.cod-estabel pallet.it-codigo pallet.data-pallet pallet.nr-pallet pallet.cod-localiz
              WITH FRAME f-main NO-ERROR.
  ...
  /* ASSIGN for Frame Fields: */
    ...
    DO:
        ASSIGN FRAME f-main pallet.cod-estabel pallet.it-codigo pallet.nr-pallet
            pallet.data-pallet pallet.cod-localiz  NO-ERROR.
    ...
    ELSE
        ...
        ASSIGN FRAME f-main pallet.data-pallet pallet.cod-localiz 
              NO-ERROR.
...
/* I-EPC005.I */
           ...
           DISABLE UNLESS-HIDDEN pallet.data-pallet pallet.cod-localiz pallet.cod-estabel pallet.it-codigo pallet.nr-pallet 
             WITH FRAME f-main.
...
/* I-EPC004.I */
        ...
        DO:                               /*  already enabled. */
            ...
            END.
                ...
                ENABLE UNLESS-HIDDEN pallet.data-pallet pallet.cod-localiz 
                  WITH FRAME f-main.
...
ON GO OF FRAME f-main OR ENTER OF FRAME f-main ANYWHERE DO:
    IF SELF:TYPE <> "editor":U OR 
       (SELF:TYPE = "editor":U AND KEYFUNCTION(LASTKEY) <> "RETURN":U) THEN DO:
        ...
        IF "pallet.data-pallet pallet.cod-localiz":U <> "":U THEN DO:
...
/*--------------------------------------------------------------------------
    File        : i-valid.i 
    Purpose     : Executa a rotina de validaªío em todas as viwers com 
                  Group-Assign-Target
    Syntax      :
    Description :
...
if avail pallet then do:
   case pallet.situacao:
       when 1 then assign fi-situacao = "N∆o Efetivado":U.
       when 2 then assign fi-situacao = "Efetivado":U.
       when 3 then assign fi-situacao = "Transferido":U.
   end case.
end.
RUN dispatch IN THIS-PROCEDURE ( INPUT 'display-fields':U ) .
if avail pallet and valid-handle(h_p-cadsi2) then do:
   /*** Desabilitar conforme situaá∆o do pallet ***/
   case pallet.situacao:
        when 1 then do:
           run enable-copia    in h_p-cadsi2 (input no).  
           run enable-modifica in h_p-cadsi2 (input yes).
           run enable-elimina  in h_p-cadsi2 (input yes).
           if valid-handle(widget-handle(c-handle-win)) THEN 
              run pi-disable in widget-handle(c-handle-win) (input no). 
        end.
        when 2 then do:
           run enable-copia    in h_p-cadsi2 (input no).   
           run enable-modifica in h_p-cadsi2 (input yes).
           run enable-elimina  in h_p-cadsi2 (input no).
           if valid-handle(widget-handle(c-handle-win)) THEN 
              run pi-disable in widget-handle(c-handle-win) (input no). 
        end.
        when 3 then do:
           run enable-copia    in h_p-cadsi2 (input no). 
           run enable-modifica in h_p-cadsi2 (input no).
           run enable-elimina  in h_p-cadsi2 (input no).
           if valid-handle(widget-handle(c-handle-win)) THEN 
              run pi-disable in widget-handle(c-handle-win) (input yes). 
        end.
   end case.
...
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
PROCEDURE pi-confirma-pallet :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
def buffer bpallet for pallet.
def var p-ok       as log  no-undo.
DEFINE VARIABLE p-largs AS CHARACTER  NO-UNDO.
if pallet.situacao = 2 then do:
   run utp/ut-msgs.p (input "show",
                      input 17006,
                      input "Pallet j† confirmado!").
   return "NOK":U.
end.
if not can-find(first it-pallet of pallet) then do:
   run utp/ut-msgs.p (input "show",
                      input 17006,
                      input "Pallet deve possuir itens para confirmar!").
   return "NOK":U.
end.
...
if param-estoq.tp-fech = 1 then
   ...
   for first estab-mat
       where estab-mat.cod-estabel = pallet.cod-estabel no-lock:
       assign data-medio = estab-mat.mensal-ate.
   end.
end.
if data-medio = ? or data-medio >= today then do:
   run utp/ut-msgs.p (input "show":U,
                      input 17006,
                      input "Data deve ser maior que a data do £ltimo mÇdio calculado!").
   return "NOK":U.
end.
FIND FIRST pol-param-estab
     WHERE pol-param-estab.cod-estabel = pallet.cod-estab NO-LOCK NO-ERROR.
/* Retirado cfme Edson solicitou (24/11/2004 - 13:40)
...
ELSE IF TODAY > pol-param-estab.data-palete THEN DO:
    RUN utp/ut-msgs.p (INPUT "show",
                       INPUT 25997,
                       INPUT "Transaá∆o n∆o permitida!" + "~~" +
                             "Verifique a Data de Paletizaá∆o informada nos ParÉmetros do Estabelecimento - POLSF008.").
    return "NOK":U.
END.
*/
run sfc\essf0903-amg.p (input rowid(pallet),
                          output p-ok, OUTPUT p-largs).
if p-ok = no THEN DO:
     RUN utp/ut-msgs.p (INPUT "show",
                       INPUT 25997,
                       INPUT "Transaá∆o n∆o permitida!" + "~~" +
                             "Verifique Larguras das bobinas paletizadas." + "(" + p-largs + ")").
    return "NOK":U.
END.
    
run sfc\essf0903d.w (input rowid(pallet),
                          output p-ok).
if p-ok = no then
   return "NOK":U.
/*Solicitado por Edson - 24/11/2004*/
find first bpallet where rowid(bpallet) = rowid(pallet) exclusive-lock no-error.
if today > pol-param-estab.data-palete  then 
   assign bpallet.data-pallet = pol-param-estab.data-palete.
else
   assign bpallet.data-pallet = TODAY.
run sfc\essf0903c.w (input rowid(pallet),
                          output p-ok).
/* validaá∆o de quantidade de emendas no pedido - Edson - Amgra - 06/12/07 */
IF pallet.nr-pedido <> 0 THEN 
    RUN sfc\essf0903a-amg.p (INPUT pallet.nr-pedido).
RUN dispatch IN THIS-PROCEDURE ('display-fields':U).
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
PROCEDURE pi-desmonta-pallet :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
if pallet.situacao = 1 then do:
   run utp/ut-msgs.p (input "show",
                      input 17006,
                      input "Pallet n∆o confirmado!" + "~~" + "Um pallet para ser desmontado antes de ser efetivado!").
   return "NOK":U.
end.
find first param-estoq  no-lock no-error. 
if param-estoq.tp-fech = 1 then
...
RUN pi-embalagem.
run sfc/essf0903e.w (input pallet.data-pallet,
                          INPUT pallet.cod-estabel, 
                          output da-data, output l-OK).
if not l-OK then
    return "NOK":U.
for each tt-movto:
    delete tt-movto.
...
blk-do:
do transaction on error undo blk-do, return "NOK":U:
    ...
    find  conta-contab where
          ...
          conta-contab.conta-contabil = param-estoq.conta-transf no-lock no-error.
    
    find item where
         item.it-codigo = pallet.it-codigo no-lock no-error.
    run utp/ut-acomp.p persistent set h-acomp.
    run pi-inicializar in h-acomp (input "Desmontando Pallet":U).
    ...
    assign tt-movto.cod-versao-integracao  = 1
           ...
           tt-movto.cod-prog-orig          = "essf0903":U
    ...
    /*Entrada das Bobinas do Pallet*/
    for each it-pallet
       where it-pallet.cod-estabel = pallet.cod-estabel
         and it-pallet.it-codigo   = pallet.it-codigo
         and it-pallet.nr-pallet   = pallet.nr-pallet no-lock:
       create tt-movto.
       assign tt-movto.cod-versao-integracao  = 1
              tt-movto.ct-codigo              = conta-contab.ct-codigo
              tt-movto.sc-codigo              = conta-contab.sc-codigo
              ...
              tt-movto.conta-contabil         = param-estoq.conta-transf
              tt-movto.dt-trans               = da-data
              tt-movto.dt-vali-lote           = it-pallet.dt-vali-lote-b
              tt-movto.nro-docto              = pallet.nro-docto
              tt-movto.serie-docto            = pallet.serie-docto
              tt-movto.cod-depos              = it-pallet.cod-depos-b
              tt-movto.cod-estabel            = it-pallet.cod-estabel
              tt-movto.it-codigo              = it-pallet.it-codigo
              tt-movto.cod-refer              = it-pallet.cod-refer-b
              tt-movto.cod-localiz            = it-pallet.cod-localiz-b
              tt-movto.lote                   = it-pallet.lote-bobina
     ...
     find  conta-contab where
          conta-contab.ep-codigo      = i-empresa and
          conta-contab.conta-contabil = c-conta-contabil no-lock no-error.
   
    for each tt-embalagem
       where tt-embalagem.quantidade > 0 no-lock:
        FIND b-item
                where b-item.it-codigo = tt-embalagem.it-codigo no-lock no-error.
        create tt-movto.  
           assign tt-movto.cod-versao-integ = 1
                  tt-movto.dt-trans         = da-data
                  tt-movto.nr-ord-produ     = 0
                  tt-movto.num-ord-inv      = 0
                  tt-movto.nro-docto        = pallet.nr-pallet
                  tt-movto.serie-docto      = ""
                  tt-movto.conta-contabil   = c-conta-contabil
                  tt-movto.it-codigo        = tt-embalagem.it-codigo
                  tt-movto.cod-estabel      = pallet.cod-estabel
                  tt-movto.cod-depos        = "ALX"
                  tt-movto.cod-localiz      = ""
                  tt-movto.cod-refer        = ""
                  tt-movto.lote             = ""
                  tt-movto.quantidade       = tt-embalagem.quantidade
                  tt-movto.un               = b-item.un
                  tt-movto.tipo-trans       = 1  /* Saida      */
                  tt-movto.esp-docto        = 31 /* requisicao */
   ...
   /* FOR EACH tt-movto.
        MESSAGE tt-movto.tipo-trans tt-movto.lote tt-movto.quantidade
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    END.*/
    /*
    for each movto-estoq
       where movto-estoq.serie-docto = pallet.serie-docto
         and movto-estoq.nro-docto   = pallet.nro-docto no-lock:
          
         if movto-estoq.cod-prog-orig <> "essf0903":U then next.
    
         create tt-movto.
         buffer-copy movto-estoq except tipo-trans cod-prog-orig dt-trans esp-docto to tt-movto.
         assign tt-movto.cod-versao-integracao = 1
                tt-movto.dt-trans              = da-data
                tt-movto.cod-prog-orig         = "essf0903":U
                tt-movto.tipo-trans            = if movto-estoq.tipo-trans = 1 then 2 else 1.
    
         if movto-estoq.esp-docto = 33 then
            assign tt-movto.esp-docto = 33 /*TRA*/.
    
         if movto-estoq.esp-docto = 28 then
            assign tt-movto.esp-docto = 5 /*DEV*/.
    end.
    */
    run pi-acompanhar in h-acomp (input "Efetivando Transaá∆o...Aguarde...":U).
...
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
PROCEDURE pi-recebe-cad-pai :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
def input param h-handle as handle no-undo.
 
assign h_p-cadsi2 = h-handle.
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
...
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
PROCEDURE pi-embalagem.
 
    DEFINE VARIABLE d-qt-ant AS DECIMAL    NO-UNDO.
    
    DEFINE VARIABLE c-emb-it-codigo AS CHARACTER  NO-UNDO.
    DEFINE VARIABLE c-codembal AS CHARACTER  NO-UNDO.
    FOR EACH tt-embalagem.
        DELETE tt-embalagem.
    END.
  ...
  /* Nova Rotina de embalagem */
    ...
    IF INT (VAR-resul.valor-char) < 999999 /*OR 
       INT (VAR-resul.valor-char) > 9999999 */
        THEN DO:
       for each estrutura
          where estrutura.it-codigo = var-result.valor-char no-lock:
           find b-item
...
for each tt-embalagem :
             ...
             IF NOT AVAIL ITEM THEN DO: 
                ...
                NEXT.
             END.
             tt-embalagem.it-codigo = c-emb-it-codigo.
...
DEFINE BROWSE br-embalagem
  QUERY br-embalagem DISPLAY
      tt-embalagem.it-codigo format "x(14)"
      tt-embalagem.descricao FORMAT "x(70)"
      tt-embalagem.quantidade
enable
         
          tt-embalagem.quantidade  
/* _UIB-CODE-BLOCK-END */
    WITH NO-ROW-MARKERS SEPARATORS SIZE 81.72 BY 5.63.
/* ************************  Frame Definitions  *********************** */
DEFINE FRAME f-embal
     /*bt-modifica AT ROW 10.58 COL 46.29*/
  /*   fi-dt-trans AT ROW 1.25 COL 16.86 COLON-ALIGNED
     ...
     c-conta-contabil AT ROW 3.29 COL 11.14 COLON-ALIGNED HELP
          "F5 para zoom"  
     c-titulo-conta AT ROW 3.29 COL 28.86 COLON-ALIGNED NO-LABEL
     br-embalagem AT ROW 4.79 COL 1.86
    /* btpeso AT ROW 10.5 COL 50 HELP "Libera Bot‰es de Embalagens"
      */
     /*bt-inclui AT ROW 10.58 COL 56*/
   /*  bt-elimina AT ROW 10.58 COL 56.57*/
     bt-ok AT ROW 12.21 COL 1.72
     bt-cancelar AT ROW 12.21 COL 12.72
     bt-ajuda AT ROW 12.21 COL 56.86
     RECT-1 AT ROW 12 COL 1
     RECT-2 AT ROW 1 COL 1
     RECT-3 AT ROW 4.63 COL 1
    WITH 1 DOWN VIEW-AS DIALOG-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
    SCROLLABLE TITLE "Devoluá∆o de Embalagens" FONT 1
             DEFAULT-BUTTON bt-ok CANCEL-BUTTON bt-cancelar
         AT COL 1 ROW 1
         SIZE 95.43 BY 15.58.
ON 'entry':U OF tt-embalagem.quantidade in browse br-embalagem 
    DO:
    
    IF NUM-RESULTS("br-embalagem") > 0 THEN
d-qt-ant = tt-embalagem.quantidade.
 
    
    END.
    ON 'leave':U OF tt-embalagem.quantidade in browse br-embalagem 
    DO:
 
              if input browse br-embalagem  tt-embalagem.quantidade > d-qt-ant then do:
                  
                      MESSAGE "Quantidade n∆o pode ser alterada para maior!"
                          VIEW-AS ALERT-BOX INFO BUTTONS OK.
                      tt-embalagem.quantidade:SCREEN-VALUE = string(d-qt-ant,"9.99999").
                       
                      apply "entry":U to tt-embalagem.quantidade in browse br-embalagem. 
                      RETURN NO-APPLY.
              END.
...
OPEN QUERY br-embalagem FOR EACH tt-embalagem.
     ...
     disable 
      ...
      find estabelec where
        ...
        if avail conta-contab then do with frame f-embal:
    ...
    WAIT-FOR "GO":U OF FRAME f-embal.
END PROCEDURE.
